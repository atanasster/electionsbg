...
const compact = (m: Metrics) => ({
  ...
  derived: m.deterministicallyDerived ?? 0,
});

const runs = readdirSync(DIR)
  .filter((f) => /^current_.+\.json$/.test(f))
  .map((file) => {
    const a = JSON.parse(readFileSync(join(DIR, file), "utf8")) as RunArtifact;
    const rows = a.rows ?? [];
      ...
      m
        ? Object.fromEntries(
            (["en", "bg"] as const).map((l) => [
              ...
              {
                n: m[l].n,
                toolAcc: m[l].toolAccuracy,
                callAcc: m[l].callAccuracy,
                argN: m[l].argumentCases,
                argAcc: m[l].argumentAccuracy,
              },
            ]),
          )
        : null;
...
/**
 * The manifest, built from the artifacts on disk. PURE — importing this module must not
 * write, or a test that imports it would silently rewrite the committed index and its
 * staleness gate would be checking its own output.
 */
