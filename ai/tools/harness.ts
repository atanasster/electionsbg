...
const run = async () => {
  ...
  const cases2: [string, string | null][] = [
    ...
    ["За какво се харчи бюджетът?", "budgetOverview"],
    ["What is the budget spent on?", "budgetOverview"],
    ["Какъв е държавният бюджет — план и изпълнение?", "budgetOverview"],
    ["Government spending by function (COFOG)?", "budgetByFunction"],
    [
      "Разходи на държавното управление по функции (COFOG)?",
      "budgetByFunction",
    ],
    ["Колко са обществените поръчки?", "procurementTotals"],
    ["Колко НПО има в България?", "ngoOverview"],
    ["Кои НПО получават най-много пари от ЕС?", "ngoTopFunded"],
    ["Кои институции дават поръчки на свързани фирми?", "ngoConflictAwarders"],
    ["Колко НПО имат сигнали за публичен интерес?", "ngoRiskSignals"],
    ["Кои НПО имат сигнал за външно финансиране?", "ngoBySignal"],
    ["В кои НПО има магистрати в ръководството?", "ngoBySignal"],
