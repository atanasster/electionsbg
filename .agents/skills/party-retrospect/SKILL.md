...
- `party` — number, nickName (Cyrillic), displayNameEn (canonical English short — use this in EN body), color, name in BG + EN
...
3. **Match place names to body language.** In the BG body use `name_bg` for every region/municipality/neighborhood (Варна, Столипиново, Пловдив). In the EN body use `name_en` (Varna, Stolipinovo, Plovdiv). Mixed scripts in the same body — e.g. "in Хасково" inside an English paragraph — read as a translation oversight. The bundle exposes both forms for regions (`name_en` / `name_bg`), neighborhoods (`name_en` / `name_bg`), and neighborhood cities (`city_en` / `city_bg`); always pick the right one.
3a. **Personal candidate names also match body language.** BG body uses the Cyrillic name (Бойко Борисов, Делян Пеевски, Фатима Йълдъс). EN body uses the transliterated English form (Boyko Borisov, Delyan Peevski, Fatima Yıldız) — pull from the bundle's `preferences.topCandidatesNational[i].name_en`, `topCandidatesByRegion[i].candidateName_en` / `leaderName_en`. Transliterations come from `public/{election}/candidates.json[].name_en` (official filings). Never write the Cyrillic form of a candidate name inside an English paragraph.
3b. **Party names match body language too.** In the BG body use the Cyrillic `nickName` (ГЕРБ-СДС, ПП-ДБ, ДПС, ПрБ, Възраждане). In the EN body use the canonical English short name from `party.displayNameEn` (GERB-SDS, PP-DB, DPS, PB, Vazrazhdane) — and use the lineage-level English short for every other party referenced in the body (rival, coalitions, predecessors). Pull the mapping from `public/canonical_parties.json` (`parties[].displayNameEn`, looked up by any `history[].nickName`). Common ones: ИТН→ITN, ПрБ→PB, ПП-ДБ→PP-DB, ПП→PP, ДБ→DB, ГЕРБ-СДС→GERB-SDS, ДПС→DPS, ДПС-НН→DPS–NB, АПС→APS, БСП-ОЛ→BSP-UL, БСП→BSP-UL, Възраждане→Vazrazhdane, МЕЧ→MECh, Величие→Velichie, Сияние→Siyanie, СБ→BB, БМ→BC, АКБ→ACB, Атака→Ataka, ВМРО→VMRO, НФСБ→NFSB, ОП→United Patriots, НДСВ→NMSP, ДОСТ→DOST, АБВ→ABV, БВ→BV, ВОЛЯ→Volya. **Never write a Cyrillic party abbreviation inside an English paragraph.**
...
9. **Risk-neighborhood section is sensitive** — describe vote counts and shares, never voters. "GERB-SDS took 31% of risk-section votes (over-index 1.2×)" (or "ГЕРБ-СДС took…" in BG) is fine. "GERB-SDS bought Roma votes" is not, even by implication.
