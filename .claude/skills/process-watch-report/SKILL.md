...
| `Eurostat — Police-recorded offences by NUTS 3 region` | `update-crime-stats` (refresh per-oblast crime rates for the My-Area crime tile) |
...
| `eurostat_crime` | `update-crime-stats` (re-runs `npx tsx scripts/crime/build_index.ts` — fetches the BG NUTS 3 slice of Eurostat's `crim_gen_reg` dataset, divides per-100K rates by 10 to per-10K, fans out BG411 to S23/S24/S25 and BG421 to PDV/PDV-00, and rewrites `data/crime/index.json`. Annual cadence — Eurostat typically refreshes April–June.) |
| `council_minutes` | `update-council-minutes` (re-runs `npm run council:scrape` — walks every município wired in `data/council/sources.json` that isn't `phase1Defer`, downloads new protokol/decision PDFs/DOCX/HTML since the per-município watermark, extracts aggregate `{for, against, abstain}` tallies + `adopted/rejected/returned/unknown` result + Решение № titles via `lib/tally.ts`, and merges into `data/council/index.json` + per-resolution shards under `data/council/{obshtina}/{YYYY}/`. Pass `--per-councillor` to also join named-vote blocks to the cacbg municipal roster, or `--ocr` for Gemini Vision fallback on scanned-only protocols.) |
