...
- Each `contract_index/<year>.json` is `{ awarders, contractors, rows }`: awarder/contractor names are dictionary-encoded (eik→name maps) and the compact row carries only the eik — the browser hook (`useContractBrowser.tsx`) rehydrates by reference so ~40k rows share a few thousand name strings (parse + memory win). `procedureBucket`/category labels resolve in the UI; English OCDS enums (`open`/`limited`/`selective`) are mapped to families in `cpvSectors.ts` (so the flat-feed years don't read as "Друга").
- `derived/breakdowns/` + `derived/contract_index/` are **gitignored** (bulky — 12k + 15 shards); they reach prod via **`bucket:sync:all`** — the `contract_index` shards (5.7–14.5 MB raw) and `concentration_full.json` are gzip-shipped by `bucket_gzip.ts` (~6–8× on the wire), so a plain `bucket:sync` leaves them uncompressed. Only `derived/sector_totals.json` is committed.
