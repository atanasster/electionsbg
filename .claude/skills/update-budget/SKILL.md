...
| `budget_law` flips / new ЗДБ year added to `LAW_DV_MATERIALS`                                              | After `budget:ingest`, also run `tsx scripts/budget/__write_izdrazhka.ts` to rebuild the per-spending-unit издръжка series; bucket:sync `data/budget/izdrazhka_by_institution.json`        |
