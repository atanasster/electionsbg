---
name: update-budget
description: Ingest Bulgarian state-budget data into data/budget/. Two main paths share one CLI — the data.egov.bg КФП feed (consolidated state-budget execution time series + monthly snapshot) and per-ministry "Отчет за изпълнението на програмния бюджет" reports (admin + program grain reconciliation against the State Budget Law). Also handles two side-ingests folded into the same data tree: Article 53 of the State Budget Law (per-municipality transfer envelope under data/budget/municipal_transfers/, emitted automatically by the main ingest from the already-fetched law HTML) and the Приложение III investment program (per-project capital allocations under data/budget/investment_program/, runs from scripts/budget/investment_program/__write_program.ts when the dv_investment_annex watcher flips). Use when the daily watcher flags `data.egov.bg бюджет`, `ministry_execution_reports`, or `dv_investment_annex` as changed, when the user asks to "refresh budget" / "update budget data", when adding a new fiscal year of execution reports or investment-program annex, after a fresh clone if `data/budget/` is empty, or to investigate a canary mismatch or sanity-warning surfaced by a previous run.
allowed-tools:
  - Read
  - Bash
  - Edit
