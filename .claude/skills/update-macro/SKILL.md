...
| Daily watcher reports `Eurostat macro (BG): new release` of **any** dataset the peer fetch reads — `gov_10a_main`, `prc_hicp_minr`, `namq_10_gdp`, `une_rt_q`, `gov_10q_ggdebt`, `gov_10q_ggnfa`, `ei_bpm6ca_q`, or `prc_hpi_q` | Run `npx tsx scripts/macro/fetch_eu_peers.ts` to refresh `data/macro_peers.json`. The script writes two things into the same file: (a) the legacy gov_10a_main 6-country (BG, EU27, RO, GR, HU, HR) revenue/expenditure/balance series + 27-member distribution that powers the `/budget` headline-card peer chips, and (b) per-indicator quarterly peer series for inflation (HICP), real GDP growth, unemployment, government debt %GDP, budget balance %GDP, current account %GDP, house-price index YoY, and youth unemployment — plus a 27-member EU rank snapshot for each direction-unambiguous indicator. These power the `/indicators` peer-overlay (Compare with EU peers toggle: snapshot table above each chart + EU27 dashed reference + RO/GR/HU/HR ghost lines on the chart itself). In practice: any Eurostat watcher flip should run `fetch_eu_peers.ts` after `fetch_eurostat.ts`. |
