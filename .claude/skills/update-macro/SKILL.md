...
| Daily watcher reports `Eurostat macro (BG): new release · nama_10_lp_ulc <date>` or `· prc_hicp_aind <date>` | Same command — `npx tsx scripts/macro/fetch_eurostat.ts`. Listed separately because these two are the productivity block's own release calendar (national-accounts revision rounds and the annual HICP, not the monthly `prc_hicp_minr`), and they are what `/indicators/economy`'s pay-vs-productivity caption is computed from. |
