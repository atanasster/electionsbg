---
name: update-macro
description: Refresh the macro indicators data (data/macro.json), the budget functional-classification artifact (data/cofog.json), and the EU peer-comparison artifact (data/macro_peers.json) — re-fetch Eurostat quarterly/annual series, World Bank WGI, COFOG (gov_10a_exp), gov_10a_main peer series, and curated tables. Use when the daily watch report flags new Eurostat releases ("Eurostat macro (BG): new release"), when the user asks to refresh macro data, when adding a new indicator, or after the curated CPI / Eurobarometer tables get a new year's value pasted in.
allowed-tools:
  - Read
  - Bash
  - Edit
...
| Daily watcher reports `Eurostat macro (BG): new release · gov_10a_main <date>` | Run `npx tsx scripts/macro/fetch_eu_peers.ts` to refresh `data/macro_peers.json` (BG + EU27 + CEE peers revenue/expenditure/balance as % of GDP, plus the 27-member distribution that powers the /budget headline-card peer chips) |
...
| `scripts/macro/fetch_eu_peers.ts` | CLI entry — fetch revenue/expenditure/balance as % of GDP for BG + EU27 + CEE peers (gov_10a_main), write `data/macro_peers.json` (5-country series + 27-member peer distribution for the /budget headline chips) |
| `scripts/watch/sources/eurostat.ts` | Daily watcher — fingerprints every tracked Eurostat dataset (quarterly macro triple, annual GDP, COFOG, gov_10a_main peer series, …) |
...
| `data/macro_peers.json` | Generated payload — gov_10a_main 5-country series (BG, EU27, RO, HU, PL) + per-naItem 27-member peer band that powers the /budget headline-card peer chips — committed |
