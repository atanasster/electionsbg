...
| Daily watcher reports `Standard Eurobarometer (institutional trust): latest wave: Standard Eurobarometer NNN ...` | A new Standard EB wave means the curated trust arrays are stale (they have no auto-fetch — plain inline constants). Open the wave's **Bulgaria national report** (europa.eu/eurobarometer → the new wave → National report: Bulgaria) and read the "trust in national government / national parliament / European Union" percentages, then paste `{ year, value }` into `EB_TRUST_GOVERNMENT`, `EB_TRUST_PARLIAMENT`, and `EB_TRUST_EU` at the top of `scripts/macro/fetch_eurostat.ts` (spring + autumn waves both map to the same `year` — use the latest reading for that year), then run `npx tsx scripts/macro/fetch_eurostat.ts`. Drives the ДОВЕРИЕ В ПРАВИТЕЛСТВОТО KPI tile on `/indicators`. |
