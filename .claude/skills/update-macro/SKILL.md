...
| Daily watcher reports `Eurostat macro (BG): new release` of **any** dataset the peer fetch reads — `gov_10a_main`, `prc_hicp_minr`, `namq_10_gdp`, `une_rt_q`, `gov_10q_ggdebt`, `gov_10q_ggnfa`, `ei_bpm6ca_q`, `prc_hpi_q`, or any of the EU compare dashboard's annual sources (`ilc_di12`, `ilc_di11`, `ilc_peps01n`, `demo_mlexpec`) | Run `npx tsx scripts/macro/fetch_eu_peers.ts` to refresh `data/macro_peers.json`. The script writes four things into the same file: (a) the legacy gov_10a_main 6-country (BG, EU27, RO, GR, HU, HR) revenue/expenditure/balance series + 27-member distribution that powers the `/budget` headline-card peer chips; (b) per-indicator quarterly peer series for inflation (HICP), real GDP growth, unemployment, government debt %GDP, budget balance %GDP, current account %GDP, house-price index YoY, and youth unemployment — plus a 27-member EU rank snapshot for each direction-unambiguous indicator (powers the `/indicators` peer-overlay + the snapshot table on `/indicators/compare`); (c) annual per-peer series for SILC Gini, S80/S20 quintile share ratio, AROPE, and life expectancy at birth — for the `/indicators/compare` inequality panel + spend-vs-outcome scatters; (d) a `wgi` top-level block: World Bank Worldwide Governance Indicators (six dimensions VA/PV/GE/RQ/RL/CC, latest year snapshot per peer + computed EU27 mean) — for the `/indicators/compare` WGI radar tile. In practice: any Eurostat or World Bank WGI watcher flip should run `fetch_eu_peers.ts` after `fetch_eurostat.ts`. |
