...
Refreshes `data/macro.json` — the per-Bulgarian-cabinet macro/governance backdrop rendered on `/governments`. Pulls quarterly Eurostat series (GDP growth, HICP inflation, unemployment, gov debt, budget balance, current account; same fiscal/external triple plus government revenue + expenditure in nominal EUR), the **authoritative annual ESA deficit ratio** (`esaBalanceAnnual`, from the EDP notification table `gov_10dd_edpt1` — the figure Eurostat headlines, e.g. BG 2025 = −3.5%), annual Eurostat GDP per capita, annual nominal GDP and net inward FDI, the **productivity block** (`nama_10_lp_ulc` → `labourProductivity` + `unitLabourCost`, both indexed 2015 = 100, and `compensationPerEmployee` as a EUR level; plus `prc_hicp_aind` → `priceIndex`, the annual HICP INDEX that deflates it), World Bank WGI, plus curated CPI / Eurobarometer trust / EU funds.

The productivity block is the first part of this artifact whose **page** — the "Производителност и разходи за труд" chart on `/indicators/economy` plus its derived pay-vs-productivity caption — can go stale independently of the headline series, so its two datasets are fingerprinted individually in `scripts/watch/sources/eurostat.ts` rather than riding the bundle's other members. Two things about it are easy to get backwards:

- **`priceIndex` is not a duplicate of `inflation`.** `inflation` is a YoY RATE rounded to 2dp; chaining a decade of rounded rates drifts from the true cumulative price level, so the caption deflates with the INDEX. Both must stay.
- **`compensationPerEmployee` is a level, not an index**, because Eurostat publishes no index form of `D1_SAL_PER`. That asymmetry is why it is not on the chart, and `payVsProductivity.test.ts` asserts it — an upstream change here needs the callout's arithmetic revisited, not just a new unit string.
...
| Daily watcher reports `Eurostat macro (BG): new release · nama_10_lp_ulc <date>`, `· prc_hicp_aind <date>` or `· earn_nt_taxwedge <date>` | Same command — `npx tsx scripts/macro/fetch_eurostat.ts`. Listed separately because these three are the productivity block's own release calendars (national-accounts revision rounds, the annual HICP rather than the monthly `prc_hicp_minr`, and tax-and-contribution policy), and the first two are what `/indicators/economy`'s pay-vs-productivity caption is computed from. `earn_nt_taxwedge` has been **frozen at 34.9% since 2018**, so a flip there is a real policy change, not a revision — worth reading rather than just re-running. |
...
| Daily watcher reports `Standard Eurobarometer (institutional trust): latest wave: Standard Eurobarometer NNN ...` | A new Standard EB wave means the curated trust arrays are stale (they have no auto-fetch — plain inline constants). Open the wave's **DATA ANNEX** (europa.eu/eurobarometer → the new wave → Data annex, EN PDF; or `GET api/survey/get/one?id=<detailId>` → the `DATANX`/`en` deliverable → `api/deliverable/download/file?deliverableId=<id>`) and read question **QA6** "How much trust do you have in certain institutions?" — the BG-row **"Tend to trust"** column for _The (NATIONALITY) Government_, _The (NATIONALITY PARLIAMENT)_ and _The European Union_. The series are **annual means of the spring + autumn waves**, so: for a completed year paste the mean of that year's two waves; the current (incomplete) year carries the latest single wave and gets re-meaned when autumn lands. Paste `{ year, value }` into `EB_TRUST_GOVERNMENT`, `EB_TRUST_PARLIAMENT`, and `EB_TRUST_EU` at the top of `scripts/macro/fetch_eurostat.ts`, then run `npx tsx scripts/macro/fetch_eurostat.ts`. Drives the ДОВЕРИЕ В ПРАВИТЕЛСТВОТО KPI tile on `/indicators`. |
| Annual TI CPI figure published | Paste the new `{ year, value }` into the curated CPI array at the top of `fetch_eurostat.ts`, then run |
...
- **Does not redownload curated tables automatically.** TI CPI and Eurobarometer trust are inline constants in `fetch_eurostat.ts` and must be updated by hand when the publishing body releases a new year — but their releases ARE now flagged: the `transparency_cpi` watcher for TI CPI, and the `eurobarometer` watcher (fingerprints the latest Standard EB wave via the EU Open Data Portal) for the `EB_TRUST_*` arrays. A flip is the prompt to paste the new figure (see the trigger rows above). `EU_FUNDS` / `EU_CONTRIBUTION` are also inline but have a known upstream (EC per-MS XLSX) — the `ec_budget_per_ms` watcher detects new editions and this skill includes a refresh procedure (see "EU funds / contribution series" above).
