...
| **B — Per-município naredba** | each município's FEES + TAX naredbi | Currently 9 wired: ТБО basis flag (9/9) · property tax for individuals (8/9 — Sofia 1.875‰, Plovdiv 1.8‰, Varna 2.0‰, Razgrad 3‰, Samokov 2.5‰, Мъглиж 3.5‰, Балчик 2.5‰, Петрич 3‰; Burgas blocked on a legacy .doc TAX-naredba) · tourist tax 7/9 (extractor walks every "1 звезда" anchor + 200-char rate-lookahead window, prefers EUR-native over BGN-converted, falls back to range form "от X до Y евро за нощ" and the legacy single-rate phrasing; Varna's tariff sits in an external Приложение № 2 not in the fetched JSON, Burgas's TAX naredba is the deferred .doc) · dog tax 2/9 (extractor walks every "куче" anchor + 200-char window for rate + година; catches inline forms — Sofia + Varna 24 BGN ≈ 12.27 EUR/година — but misses tabular dual-currency rows in Plovdiv/Razgrad/Maglizh/Balchik/Petrich, which need column-aware extraction). Other oblast capitals land as parsers are added; the `scripts/probe_obshtini_all.ts` survey enumerates obshtini.bg coverage for the remaining 257 municípios. | Annual (Dec→Jan adoption) | Per-município parser via `createObshtiniBgNaredbaParser` factory when both naredbi are on the obshtini.bg platform; bespoke for direct PDFs |
