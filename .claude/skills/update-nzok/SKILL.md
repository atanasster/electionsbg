...
3. Sanity-check the console output. Hospital payments print, in decreasing order of urgency: any **skipped** month (expected 0 — a skip is a month the site does not have); any **restated** rows (money already published changing value — the loader writes the `data-changes.json` entry, you commit and sync it); any month that **repeats** its predecessor with a zero month column (published as filed, see RC-5); and finally the months that loaded with something **unverified** — blocks disagreeing with НЗОК's own printed facility count, or printing no subtotal at all. The count figure is descriptive — watch it for a TREND against the baseline the loader prints, because a rise means НЗОК's documents changed shape. The **no-subtotal** figure beside it is not descriptive: money in a block that prints no subtotal is reconciled by nothing but the whole-file 0.5% ratio, which is the check that let €1,672,123 through before this work. Drugs print the €total + top INN + oncology group L; execution prints revenue + expenditure YTD.

   Legacy note: this step used to say "reconcile the printed facility count + national total against the file's own Общо РЗОК grand total". That whole-file check is now the BACKSTOP rather than the check — each РЗОК block is reconciled against its own printed subtotal, and the facility count is reported rather than asserted. Nothing is left for you to reconcile by eye; the loader either accepts a month or names the block that failed.
4. Commit the changed `data/budget/nzok/*.json` — **and `data/data-changes.json` if the loader wrote a restatement entry** (+ `data/db/procurement.lock.json` if you pushed).
5. **`bucket:sync data/budget/nzok/`** — the budget/drug/execution JSONs are served from the GCS bucket; without the sync those tiles aren't live. If step 4 committed `data/data-changes.json`, sync it too (`npm run bucket:sync:paths -- budget data-changes.json`) — it is bucket-served from the data root, so the corrected figures otherwise publish while the note explaining them does not.
...
### Private-hospital ГФО revenue + public/private band — `npm run data:nzok -- --revenue` (opt-in, heavy OCR)

Public/state hospitals report quarterly to МЗ (ЕЕОФ → `hospital_financials.json`);
private hospitals do **not**, but as commercial companies they file an annual ГФО
with the Търговски регистър. `--revenue` recovers each private hospital's annual
revenue (2019–2024) so the whole hospital universe has a comparable multi-year
financial picture, and the EC-lawsuit "% publicly funded" number gets a real
denominator. Two generators run in order:

- `write_hospital_revenue.ts` → **`hospital_revenue.json`** (127 hospitals, ~611
  hospital-years). Discovers each ГФО `ActID` from the TR daily open-data feed
  already on disk (`raw_data/tr/daily/*.json`) → `curl`s the CC-BY PDF from
  `portal.registryagency.bg/CR/api/Documents/{ActID}` (node fetch is WAF-blocked;
  curl passes) → Gemini Vision OCR of the ОПР (код 18000 „Общо приходи“ / 15100) →
  sanity gate dropping a read where same-year НЗОК > 1.15× revenue. **Resumable** —
  a re-run only fills cells still empty, so refreshing FY2023/24 as hospitals file
  late is cheap.
- `write_public_private.ts` → **`public_private.json`** — the "Частни болници и
  обществените поръчки" band blob: ownership split of НЗОК money + the 50%-threshold
  stats + who runs ЗОП tenders. Joins ownership + reimbursement-by-EIK + revenue +
  the PG contracts corpus.

Prereqs: `GEMINI_API_KEY` in `.env.local` + the **local Postgres** (the tender
counts + the sanity gate). Upstream source is the Търговски регистър (the same
`egov_commerce` daily feed behind `tr:daily-refresh`), but the ГФО filings are
**annual/late** and the OCR is expensive — so this is a **periodic manual refresh**
(run after the Sept filing season, or whenever the private-hospital financials are
being refreshed), NOT a daily-watcher auto-run. Ingest details: `scripts/nzok/README.md`.
Then for production: `bucket:sync data/budget/nzok/`.
