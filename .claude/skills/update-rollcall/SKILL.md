...
| **Data-diet note** | The `derived/per-mp/<ns>/<csvId>.json` shard (loyalty + attendance + similarity + cohort + dissents in one ~2–4 KB file) is what the `/candidate/:id` voting & twins tiles read. When it's missing, the page falls back to the chamber-wide aggregates — `similarity.json` is **~12 MB**, `loyalty.json` ~416 KB. The candidate page now gates that fallback on NS membership (an MP *in the selected NS* with a missing shard still pulls the 12 MB; an off-NS / former MP skips it), so a regular ingest+`bucket:sync` keeping the shards fresh matters. See "Per-MP shard invariant" in process-watch-report and [[project_candidate_page_data_diet]]. |
