...
const SECTOR_PAGES: SectorPageContent[] = [
  ...
  },
  {
    ...
    en: {
      ...
      intro:
        "The Ministry of Regional Development and Public Works (МРРБ) is a pass-through ministry: it directs about €1.06 billion a year, but only ~€100 million flows through its own public procurement — the rest leaves as capital transfers to municipalities and co-financing of EU cohesion policy. This page follows the money to where it actually lands, per oblast: the absorption of ОП „Региони в растеж“ 2014-2020 (closed, ~96%) versus Програма „Развитие на регионите“ 2021-2027 (~20% — the absorption risk: expenditure is eligible until 31 December 2029, and the n+3 rule forfeits unspent money tranche by tranche each year), cohesion euros per capita by oblast, spending on housing and community amenities versus the EU (COFOG GF06), the Geodesy, Cartography and Cadastre Agency (АГКК), the National Building Control Directorate (ДНСК) and the 28 regional governor administrations. Does the money reach the poorest oblasts (Kardzhali, Montana, Vidin, Silistra) or concentrate where capacity already exists? Roads (АПИ) and water (ВиК) are separate sectors.",
    },
  },
  {
    id: "social",
...
<p>Source: <a href="https://kolkostruva.bg/opendata" rel="nofollow noopener">CPC — "How Much Does It Cost" (open data)</a>.</p>`.trim(),
  ...
  staticPage({
    ...
    ogImage: "/og/consumption-categories.png",
    bodyHtml: `
...
<p>Source: <a href="https://kolkostruva.bg/opendata" rel="nofollow noopener">CPC — "How Much Does It Cost" (open data)</a>.</p>`.trim(),
  ...
  staticPage({
    ...
    ogImage: "/og/consumption-deals.png",
    bodyHtml: `
...
<p>Source: <a href="https://kolkostruva.bg/opendata" rel="nofollow noopener">CPC — "How Much Does It Cost" (open data)</a>.</p>`.trim(),
  ...
  staticPage({
    ...
    ogImage: "/og/consumption-unit-prices.png",
    bodyHtml: `
...
<li><a href="${SITE_URL}/companies?political=1">Списък на всички фирми</a>, свързани с лице в публичния живот. Наборът е много по-широк от депутатите: най-многобройни са кандидатите на избори и общинските съветници, следвани от министри, депутати и магистрати.</li>
...
<li><a href="${SITE_URL}/en/companies?political=1">List of all companies</a> linked to a person in public life. The set is far wider than MPs alone: election candidates and municipal councillors are the largest groups, followed by ministers, MPs and magistrates.</li>
...
<li><a href="${SITE_URL}/funds/calls">Отворени процедури</a> — по какво може да се кандидатства сега: краен срок, бюджет, допустими кандидати.</li>
...
<li><a href="${SITE_URL}/en/funds/calls">Open calls</a> — what you can apply for right now: deadline, budget, eligible applicants.</li>
...
<p>The Recovery and Resilience Plan is the newest instrument, so its absorption is below the corpus average — that is age, not performance.</p>`.trim(),
  ...
  staticPage({
    ...
    bodyHtml: `
<h1>Бенефициенти по европейските програми</h1>
<p>Организациите с поне един договор в регистъра на бенефициентите на ИСУН 2020, подредени по договорена стойност.</p>
<h2>Какво означават числата</h2>
...
<p>The „linked to an MP" label is a <strong>signal to check, not a conclusion</strong>: it marks an EIK match against a declared interest, not an irregular contract.</p>`.trim(),
  ...
  staticPage({
    ...
    bodyHtml: `
<h1>Оперативните програми</h1>
<p>Програмите в ИСУН 2020, подредени по договорена стойност. Изберете програма, за да видите нейните процедури и договори.</p>
...
<p>The corpus covers the 2014-2020 and 2021-2027 operational programmes, the Recovery and Resilience Plan, the EEA/Norway financial mechanism and several national funds — hence „ИСУН 2020 contracts" rather than „EU funds".`.trim(),
  ...
  staticPage({
    ...
    // prerendered HTML below is deliberately the DESCRIPTION of the register, never a list of
    // the calls themselves: a static snapshot of live deadlines would serve expired calls as
    // open the moment a crawl lands, which is precisely the harm the query-time status
    // derivation exists to prevent (migration 142's header). The rows come from
    // /api/db/open-calls client-side.
    //
    // There are NO per-call `<loc>`s for the same reason, and because an individual procedure
    ...
    // Generated by scripts/og/capture-screens.ts (`funds-calls`), whose spec waits on the
    // loaded TABLE rather than the heading, so the card never captures the skeleton.
    ogImage: "/og/funds-calls.png",
    bodyHtml: `
<h1>Отворени процедури по европейски програми</h1>
...
<p>Кандидатстването става в съответния регистър — всеки ред води към него. Източници: <a href="https://eumis2020.government.bg/bg/s/Procedure/Active" rel="nofollow noopener">ИСУН 2020</a> и <a href="https://sp2023.bg" rel="nofollow noopener">sp2023.bg</a> (ДФ „Земеделие“).</p>`.trim(),
    english: {
      ...
      bodyHtml: `
<h1>Open calls under EU programmes</h1>
...
<p>Извън залата: <a href="${SITE_URL}/governance/declarations">декларациите</a> пред Сметната палата, <a href="${SITE_URL}/mp-assets">декларираното имущество</a>, <a href="${SITE_URL}/companies?political=1">фирмите на лица в публичния живот</a> и <a href="${SITE_URL}/connections">графът на връзките</a>.</p>`.trim(),
...
<p>Outside the chamber: <a href="${SITE_URL}/en/governance/declarations">declarations</a> filed with the Court of Audit, <a href="${SITE_URL}/en/mp-assets">declared assets</a>, <a href="${SITE_URL}/en/companies?political=1">companies of people in public life</a>, and the <a href="${SITE_URL}/en/connections">connections graph</a>.</p>`.trim(),
      extraJsonLd: [
        buildFaqLd([
          {
