...
export const SOURCE_GROUPS: SourceGroupDef[] = [
  ...
  },
  {
    ...
    desc: {
      bg: "Котвите на данъчния симулатор: ставките в държавите от ЕС за сравнителите „Като в…“ (PwC Worldwide Tax Summaries), дяловете за отбрана от компендиума на НАТО, прогнозата на ЕК за България, върху която стъпва 5-годишната проекция на салдото и дълга, плюс котвите на динамичния (поведенчески) режим — докладът на ЕК за несъбраното ДДС и МВФ изданията зад фискалните мултипликатори. Стойностите живеят като константи с източници в кода на симулатора и се обновяват ръчно при сигнал от наблюдателя.",
      en: "The tax simulator's anchors: EU member-state rates for the \"Like in…\" comparators (PwC Worldwide Tax Summaries), defence shares from the NATO expenditure compendium, the EC forecast for Bulgaria that the 5-year balance/debt projection is built on, plus the dynamic (behavioral) mode's anchors — the EC VAT gap report and the IMF vintages behind the fiscal multipliers. The values live as sourced constants in the simulator code and are updated manually when the watcher flags a change.",
    },
