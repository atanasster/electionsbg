...
export const SOURCE_GROUPS: SourceGroupDef[] = [
  ...
  },
  {
    ...
    desc: {
      bg: "Котвите на данъчния симулатор: ставките в държавите от ЕС зад сравнителите „Като в…“ по отделните лостове и зад бутоните за избор на цяла държава (PwC Worldwide Tax Summaries за ДДС/ДДФЛ/корпоративен; Tax Foundation и таблиците на ЕК DG TAXUD за акцизите — дизел/бензин/цигари/спиртни напитки/вино), дяловете за отбрана от компендиума на НАТО, прогнозата на ЕК за България, върху която стъпва 5-годишната проекция на салдото и дълга, плюс котвите на динамичния (поведенчески) режим — докладът на ЕК за несъбраното ДДС и МВФ изданията зад фискалните мултипликатори. Котвите за хазарта (GGR) и виното са по данни на бранша и НАП. Стойностите живеят като константи с източници в кода на симулатора и се обновяват ръчно при сигнал от наблюдателя.",
      en: "The tax simulator's anchors: EU member-state rates behind both the per-lever \"Like in…\" comparators and the whole-country quick-select profiles (PwC Worldwide Tax Summaries for VAT/PIT/corporate; Tax Foundation and the EC DG TAXUD tables for the excise rates — diesel/petrol/cigarettes/spirits/wine), defence shares from the NATO expenditure compendium, the EC forecast for Bulgaria that the 5-year balance/debt projection is built on, plus the dynamic (behavioral) mode's anchors — the EC VAT gap report and the IMF vintages behind the fiscal multipliers. The gambling (GGR) and wine bases are industry/НАП-reported. The values live as sourced constants in the simulator code and are updated manually when the watcher flags a change.",
    },
