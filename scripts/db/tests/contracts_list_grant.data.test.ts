...
const skipRole =
  !haveDb || !hasRole
    ? "Postgres unreachable, or the app_readonly role is absent — run npm run db:pg:bootstrap"
    : false;
reportSkip(import.meta.url, skipRole);

test.skipIf(skipRole)(
  "rebuild_contracts_list() grants contracts_list — with the default-privilege backstop OFF",
  async () => {
...
test.skipIf(skipRole)(
  "rebuild_contract_risk_cache() grants risk_upheld_ocid",
  async () => {
