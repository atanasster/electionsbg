...
const skipBodies = !bodiesLoaded
  ...
  : false;
const skipAgri = !agriLoaded
  ? "Postgres unreachable, or agri_subsidies is empty — run npm run db:load:agri:pg"
  : false;
const skipDrugs = !drugsLoaded
  ? "Postgres unreachable, or nzok_drug_quarterly is empty — run npm run db:load:nzok-drug-quarterly:pg"
  : false;
const skipActivities = !activitiesLoaded
  ? "Postgres unreachable, or nzok_activities is empty — run npm run db:load:nzok-activities:pg"
  : false;
reportSkip(import.meta.url, skipBodies);
reportSkip(import.meta.url, skipAgri);
reportSkip(import.meta.url, skipDrugs);
reportSkip(import.meta.url, skipActivities);

test.skipIf(skipDrugs)(
  "every reimbursed molecule resolves to a servable /molecule page",
  async () => {
...
test.skipIf(skipDrugs)(
  "the molecule payload keeps BOTH tiers distinguishable",
  async () => {
...
test.skipIf(skipDrugs)("an unknown INN still resolves to NULL", async () => {
...
test.skipIf(skipDrugs)(
  "the overpay tier never hands the client a null rows list",
  async () => {
...
test.skipIf(skipDrugs)(
  "the INN fold tolerates the whitespace a URL can carry",
  async () => {
...
test.skipIf(skipActivities)(
  "every procedure the search can offer has activity rows behind it",
  async () => {
    ...
    assert.equal(
      orphan.n,
      "0",
      "nzok_activities carries a blank procedure code",
    );
  },
);

test.skipIf(skipAgri)(
  "the agri beneficiary dimension is BUILT, not just created",
  async () => {
...
test.skipIf(skipAgri)(
  "the agri typeahead never offers a row with no /farm page",
  async () => {
...
test.skipIf(skipAgri)(
  "the agri typeahead is not a per-keystroke table scan",
  async () => {
...
test.skipIf(skipBodies)(
  "every judicial body the search offers has a /court page",
  async () => {
    const [row] = await allRows<{ total: string; servable: string }>(`
