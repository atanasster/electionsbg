...
const skipRoles =
  !haveDb || !haveRoles
    ...
    : false;
const skipSeats =
  !haveDb || !haveRoles || !haveSeats
    ? "Postgres unreachable, or person_role / mp_seat is absent — run npm run db:resolve:persons and npm run db:load:rollcall:pg"
    : false;
...
test.skipIf(skipSeats)(
  "5.0 — no MP row carries another person's seat",
  async () => {
...
test.skipIf(skipSeats)(
  "5.0b — the stored party matches the seat's own name",
  async () => {
...
test.skipIf(skipSeats)(
  "5.3 — the ENTRY group, never mp_seat.party_id (last-seen)",
  async () => {
...
test.skipIf(skipRoles)(
  "5.1 — coverage floor, and the crosswalk covers every live group short",
  async () => {
    const [{ n }] = await allRows<{ n: string }>(
    ...
    assert.ok(
      shorts.length > 20,
      `only ${shorts.length} distinct group shorts`,
    );
...
test.skipIf(skipBrowse)(
  "the ПАРТИЯ column's multi-party badge stays consistent with party_codes",
  async () => {
...
test.skipIf(skipRoles)(
  "5.2 — one vocabulary: no Cyrillic, every id labellable",
  async () => {
