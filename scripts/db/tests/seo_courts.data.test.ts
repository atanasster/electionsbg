...
test.skipIf(skipBodies)(
  "enumerates every judicial body — the QUERY runs and returns all of them",
  async () => {
...
test.skipIf(skipBodies)(
  "numeric fields are numbers, not node-postgres numeric strings",
  async () => {
...
test.skipIf(skipBodies)(
  "sourcesBuilt is true on a loaded database, and the workload split is real",
  async () => {
    const rows = await readSeoCourts();
...
test.skipIf(skipBodies)(
  "the latest year matches the series end it is reported alongside",
  async () => {
...
test.skipIf(skipBodies)(
  "magistrate counts agree with judicial_body_detail()",
  async () => {
