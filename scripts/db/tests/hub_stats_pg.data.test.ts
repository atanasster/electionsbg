...
test.skipIf(skipBlob)(
  ...
  async (t) => {
    ...
    try {
      ...
      reportSkip(
        import.meta.url,
        "the rollcall matviews are not built here — run npm run db:load:rollcall-derived:pg",
      );
      return t.skip();
    }
    if (sql.size === 0) {
      reportSkip(
        import.meta.url,
        "readHubStatsFromPg returned nothing — the rollcall matviews are empty",
      );
      return t.skip();
    }
