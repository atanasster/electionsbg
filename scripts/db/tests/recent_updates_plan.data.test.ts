...
test.skipIf(skipDb)("the deployed body carries NOT MATERIALIZED", async () => {
...
test.skipIf(skipDb)(
  "no branch reads more rows than its recent window holds",
  async () => {
...
test.skipIf(skipDb)(
  "the cutoff reaches the index as a condition, on every ordering column",
  async () => {
    const plan = await planFor(1, 200);
...
test.skipIf(skipDb)(
  "the ceiling shape stays inside the 10 s statement_timeout budget",
  async () => {
