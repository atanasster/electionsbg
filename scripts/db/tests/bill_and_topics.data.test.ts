...
const skipSessions = !existsSync(SESSIONS)
  ? "data/parliament/votes/sessions absent — it is committed, so this is a sparse checkout"
  : false;
...
test.skipIf(skipDb)(
  "no bill claims an adoption the corpus cannot support",
  async () => {
...
test.skipIf(skipDb)(
  "every bill_id on a vote_item resolves, and only readings carry one",
  async () => {
    ...
    assert.equal(
      Number(orphans.n),
      0,
      "vote_item rows point at a missing bill",
    );
...
test.skipIf(skipDb)(
  "no bill points at a first reading the chamber took back",
  async () => {
...
test.skipIf(skipDb || skipSessions)(
  "a reload REUSES the bill ids the database already holds",
  async () => {
    ...
    assert.ok(
      stored.size > 0,
      "bill is empty — run npm run db:load:rollcall:pg",
    );
...
test.skipIf(skipDb || skipSessions)(
  "the route's SQL outcome buckets agree with the TypeScript classification",
  async () => {
...
test.skipIf(skipDb)(
  "zero-cast items are excluded from the day summary, not bucketed as contested",
  async () => {
...
test.skipIf(skipDb)(
  "the day summary filters re-votes, and the contested feed ranks by closeness",
  async () => {
