...
const skipDay =
  !haveDb || !existsSync(FILE)
    ? "Postgres unreachable, or that day's committed session file is absent — sparse checkout"
    : false;
reportSkip(import.meta.url, skipDb);
reportSkip(import.meta.url, skipDay);

test.skipIf(skipDay)(
  "the day route reproduces the session file's agenda and tallies",
  async (t) => {
    ...
    if (rows.length === 0) {
      reportSkip(
        import.meta.url,
        `vote_item has no row for ${DAY} — run npm run db:load:rollcall:pg`,
      );
      return t.skip();
    }
...
test.skipIf(skipDay)(
  "one item's per-MP votes reproduce the file, modulo the duplicate casts",
  async (t) => {
    const file = readDay();
    ...
    if (!first) {
      reportSkip(
        import.meta.url,
        `the committed session file for ${DAY} carries no item with votes`,
      );
      return t.skip();
    }
    ...
    if (!ids.length) {
      reportSkip(
        import.meta.url,
        `vote_item has no row for that ${DAY} item — run npm run db:load:rollcall:pg`,
      );
      return t.skip();
    }
...
test.skipIf(skipDb)(
  "neither route can be planned into the seq scan",
  async () => {
