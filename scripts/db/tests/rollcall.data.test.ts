...
const skipItems =
  !haveDb || !(await tableExists("vote_item"))
    ? "Postgres unreachable, or vote_item is absent — run npm run db:load:rollcall:pg"
    : false;
const skipCasts =
  !haveDb || !(await tableExists("vote_cast"))
    ? "Postgres unreachable, or vote_cast is absent — run npm run db:load:rollcall:pg"
    : false;
const skipSeats =
  !haveDb || !(await tableExists("mp_seat"))
    ? "Postgres unreachable, or mp_seat is absent — run npm run db:load:rollcall:pg"
    : false;
const skipAttendance =
  !haveDb || !(await tableExists("mp_attendance"))
    ? "Postgres unreachable, or mp_attendance is absent — run npm run db:load:rollcall-derived:pg"
    : false;
const skipDissent =
  !haveDb || !(await tableExists("mp_dissent"))
    ? "Postgres unreachable, or mp_dissent is absent — run npm run db:load:rollcall-derived:pg"
    : false;
const skipSimilarity =
  !haveDb || !(await tableExists("mp_similarity"))
    ? "Postgres unreachable, or mp_similarity is absent — run npm run db:load:rollcall-derived:pg"
    : false;
const skipCohesion =
  !haveDb || !(await tableExists("party_cohesion"))
    ? "Postgres unreachable, or party_cohesion is absent — run npm run db:load:rollcall-derived:pg"
    : false;
reportSkip(import.meta.url, skipItems);
reportSkip(import.meta.url, skipCasts);
reportSkip(import.meta.url, skipSeats);
reportSkip(import.meta.url, skipAttendance);
reportSkip(import.meta.url, skipDissent);
reportSkip(import.meta.url, skipSimilarity);
reportSkip(import.meta.url, skipCohesion);

test.skipIf(skipItems)(
  "the corpus is loaded, and the dedupe left the right split",
  async () => {
    ...
    // filter superseded_by IS NULL and so see the 15,113 the JSON artifacts were computed
    ...
    assert.equal(Number(r.total), 16760, "raw item count moved");
    assert.equal(
      Number(r.stand),
      15113,
      "the standing set is not the deduped set",
    );
    assert.equal(Number(r.superseded), 1647);
  },
);

test.skipIf(skipItems)(
  "every superseded item points at a survivor on its own day",
  async () => {
...
test.skipIf(skipCasts)("no orphan casts", async () => {
...
test.skipIf(skipSeats)(
  "the 26 recycled mp_ids are exactly these, and (ns, mp_id) keeps them apart",
  async () => {
...
test.skipIf(skipCasts)(
  "party is recorded per CAST, and 179 seats change it mid-term",
  async () => {
    const r = await one<{ n: string }>(
      `SELECT count(*) n FROM (
...
test.skipIf(skipItems)(
  "the standing set matches index.json, per parliament",
  async () => {
    ...
    assert.equal(total, 614, "plenary-day count moved");
  },
);

test.skipIf(skipAttendance)(
  "mp_attendance reproduces attendance.json, except where the source double-counts",
  async () => {
...
test.skipIf(skipSimilarity)(
  "mp_similarity reproduces similarity.json's cosine",
  async () => {
    const sim = (
      JSON.parse(
...
test.skipIf(skipDissent)(
  "mp_dissent groups on the CAST-time party, not the seat's last-seen one",
  async () => {
...
test.skipIf(skipAttendance)(
  "every matview filters the superseded re-votes",
  async () => {
...
test.skipIf(skipCohesion)(
  "party_cohesion reproduces cohesion.json, item-weighted",
  async () => {
    const coh = (
      JSON.parse(
...
test.skipIf(skipDissent)(
  "mp_dissent reproduces dissents.json per member",
  async () => {
    const dj = (
      JSON.parse(
...
test.skipIf(skipCasts)(
  "the live-served shapes stay under the Cloud SQL buffer budget",
  async () => {
