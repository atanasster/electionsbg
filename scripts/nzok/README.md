...
| `hospital_payments.json`             | `write_hospital_payments.ts`              | `/bg/hospitals/bmp/{year}` PDF                                                                   | Latest monthly per-hospital БМП. `pdftotext -layout`, wrap-tolerant, per-РЗОК-block reconciliation + a Рег.№ universe assert (the count is reported, not asserted). Each row carries `eik` from the crosswalk. **Now the crosswalk universe + parity net — the tile is PG-served** (see Shipped below). ~90 KB (381 facilities).                                                                                                                                                                                                                                           |
...
Its completeness model changed on 2026-08-25
(`docs/plans/nzok-hospital-parser-hardening-v1.md`, Tier 2). НЗОК prints a subtotal
above each РЗОК block, so **each block is reconciled against its own subtotal** —
an absolute band, so a €129 error inside a €51m block is visible where the old
whole-file 0.5% ratio could not see it, and a failure names the block rather than
the document. The **Рег.№ universe** is asserted separately (every facility the
document prints must reach a row), which is what catches a dropped ZERO-payment
row — it moves no money, so no money check can. НЗОК's own facility count is
**reported, never asserted** (`countMismatches`, with the „№ по ред" ordinals that
are absent): the number counts LISTED facilities in 2023 and PAID ones from 2024,
and on two devices files it is simply the БМП report's Sofia figure.
