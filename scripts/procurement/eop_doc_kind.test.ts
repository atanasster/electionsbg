...
describe("classifyDocName", () => {
  ...
  test.each([
    [
      "Приложение_1_Техническа спецификация_Хранителни стоки за служеби офиси_2026.pdf",
      "spec",
    ],
    ...
    [
      "Приложение_2_Методика за оценка_Хранителни стоки за служебни офиси_2026.pdf",
      "methodology",
    ],
    ["Приложение_5.1_Договор_ОП1_храни_2026.pdf", "contract_draft"],
    ["ПРОЕКТ -Договор охрана.docx", "contract_draft"],
    [
      "Документация_Хранителни стоки за служебни офиси_2026.pdf",
      "documentation",
    ],
  ...
  test("spec wins over the generic patterns it co-occurs with", () => {
    expect(
      classifyDocName("Приложение № 1 - Техническа спецификация.pdf"),
    ).toBe("spec");
    expect(
      classifyDocName("Техническа спецификация към проект на договор.pdf"),
    ).toBe("spec");
  });
...
describe("isExtractable", () => {
  test("office formats yes, archives and images no", () => {
    for (const e of [".pdf", ".doc", ".docx", ".PDF"])
      expect(isExtractable(e)).toBe(true);
...
describe("pickSpec", () => {
  ...
  test("breaks ties on size — the substance is in the bigger file", () => {
    const got = pickSpec([
      {
        Name: "Техническа спецификация - приложение.pdf",
        Extension: ".pdf",
        Size: 10,
      },
  ...
  test("a spec inside an archive is NOT picked — tier B cannot extract it", () => {
    expect(
      pickSpec([
        { Name: "Техническа спецификация.zip", Extension: ".zip", Size: 999 },
      ]),
    ).toBeNull();
  });
