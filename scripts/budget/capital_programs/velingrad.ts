// Rollup parser for Велинград's annual капиталова програма. Consumes
// the Gemini Vision OCR JSON and emits a tile-ready
// VelingradCapitalProgramFile.
//
// Velingrad obshtina = PAZ08 (Pazardjik oblast), EKATTE 10450 (the
// city). 21 settlements: city + 20 villages.
//
// Run: tsx scripts/budget/capital_programs/velingrad.ts [--year 2025]
...
const SOURCE_URLS: Record<number, string> = {
  2025: "https://m.velingrad.bg/wp-content/uploads/2025/04/ПРОЕКТ-ПКР-2025.pdf",
};
...
interface VelingradCapitalProject {
  id: number;
  name: string;
...
interface VelingradCapitalSettlementRollup {
  name: string;
  projectCount: number;
  total: Money;
...
interface VelingradCapitalProgramFile {
  fiscalYear: number;
  generatedAt: string;
  source: {
    publisher: string;
...
// 21 settlements of Община Велинград (PAZ08).
const VLG_SETTLEMENTS: Array<{ name: string; prefix: "гр." | "с." }> = [
  { name: "Велинград", prefix: "гр." },
  { name: "Абланица", prefix: "с." },
  { name: "Алендарова", prefix: "с." },
  { name: "Биркова", prefix: "с." },
  { name: "Бозьова", prefix: "с." },
  { name: "Бутрева", prefix: "с." },
  { name: "Всемирци", prefix: "с." },
  { name: "Враненци", prefix: "с." },
  { name: "Горна Биркова", prefix: "с." },
  { name: "Горна Дъбева", prefix: "с." },
  { name: "Грашево", prefix: "с." },
  { name: "Долна Дъбева", prefix: "с." },
  { name: "Драгиново", prefix: "с." },
  { name: "Кандови", prefix: "с." },
  { name: "Кръстава", prefix: "с." },
  { name: "Пашови", prefix: "с." },
  { name: "Рохлева", prefix: "с." },
  { name: "Света Петка", prefix: "с." },
  { name: "Цветино", prefix: "с." },
  { name: "Чолакова", prefix: "с." },
  { name: "Юндола", prefix: "с." },
];

const SETTLEMENT_PATTERNS = VLG_SETTLEMENTS.slice()
...
const main = () => {
  ...
  if (!existsSync(ocrPath)) {
    throw new Error(
      `Missing OCR JSON at ${ocrPath} — run velingrad_ocr.ts --year ${fiscalYear} first`,
    );
  }
  const ocr: OcrFile = JSON.parse(readFileSync(ocrPath, "utf-8"));
  console.log(
    `[velingrad-capital] reading ${ocrPath} (year ${fiscalYear}, ${ocr.projects.length} OCR rows)`,
  );
  ...
  const bySettlementAgg = new Map<
    string,
    { total: number; projects: VelingradCapitalProject[] }
  >();
  ...
  const bySettlement: VelingradCapitalSettlementRollup[] = [...bySettlementAgg]
    .map(([name, agg]) => ({
      name,
  ...
  const out: VelingradCapitalProgramFile = {
    fiscalYear,
    ...
    municipalityCode: "PAZ08",
    municipalityNameBg: "Велинград",
    municipalityNameEn: "Velingrad",
    currency: "BGN",
  ...
  console.log(
    `[velingrad-capital] wrote ${outPath} — ${projects.length} projects, itemised EUR ${(
  ...
  console.log(
    `[velingrad-capital] settlement tagging: ${tagged}/${projects.length} (${(
      (100 * tagged) /
      Math.max(projects.length, 1)
    ).toFixed(0)}%)`,
  );
  console.log("[velingrad-capital] top settlements:");
