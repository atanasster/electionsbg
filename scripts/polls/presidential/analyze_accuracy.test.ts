...
const summaryFile = path.join(
  REPO_ROOT,
  "data/2021_11_14_pvr/national_summary.json",
);
const ticketsFile = path.join(REPO_ROOT, "data/2021_11_14_pvr/tickets.json");
const has2021Corpus = fs.existsSync(summaryFile) && fs.existsSync(ticketsFile);
const summary2021 = has2021Corpus
...
describe.runIf(has2021Corpus)(
  "computeCycleAccuracy — real 2021 tree, synthetic poll (no real presidential poll exists for this cycle yet)",
  () => {
...
describe.runIf(has2021Corpus)(
  "main — real corpus (no cycle stamped yet, so nothing to score)",
  () => {
