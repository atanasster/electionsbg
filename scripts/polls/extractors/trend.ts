...
export const extractTrend = async (
  ...
  if (classifyRace(title) !== "parliamentary") {
    throw new Error(
      `extractTrend(${pubId}): title alone resolves to a non-parliamentary race — ` +
        `presidential extraction is not built for TR yet (Tier 4b)`,
    );
  }
  ...
  // plans to reuse it for TR's own presidential captures (decision 10's
  ...
  if (race !== "parliamentary") {
    throw new Error(
      `extractTrend(${pubId}): classifyRace resolved "${race}" for this capture — ` +
        `presidential extraction is not built for TR yet (Tier 4b)`,
    );
  }
