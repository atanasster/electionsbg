// МОН (Министерство на образованието и науката) — Национално външно оценяване
// (НВО) 7th-grade results, published via data.egov.bg. Fingerprints the dataset's
// resource UUID list; when МОН uploads a new year's CSV the list grows (or a UUID
// changes) and the watcher fires. НВО is the prior-attainment baseline for the
// value-added ("напредък 7→12 клас") on /school/:id, so a new НВО year should feed
// the update-schools pipeline. It usually publishes alongside ДЗИ each summer, but
// this gives it an independent trigger rather than relying on the ДЗИ watcher.
...
export const indicatorsMonNvo: WatchSource = {
  id: "indicators_mon_nvo",
  label: "МОН: НВО 7. клас резултати (data.egov.bg)",
  url: DATASET_PAGE,
  // Annual cadence (one new year per summer, together with ДЗИ).
  cadence: "daily",
