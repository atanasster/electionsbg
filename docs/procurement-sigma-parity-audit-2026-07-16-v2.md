...
- **✅ Shipped & verified.** `scripts/db/schema/pg/079_contracts_cais_id.sql` adds `cais_id` as a STORED generated column `= CASE WHEN unp present THEN unp; WHEN ocid LIKE 'eop-T%' THEN substring(ocid,5); WHEN ocid LIKE 'ocds-e82gsb-%' THEN 'T'||substring(ocid,13) END`, wired into `load_pg.ts` (applied like `078`) so every reload recomputes it (`contracts_stage` is `LIKE contracts INCLUDING GENERATED`). Population on the corpus: 258,446 rows keep the standard УНП, **10,010 rows / €4.43 bn recover a `T`-id**, 75,255 (pre-ЦАИС legacy) stay NULL. Proven rows map exactly: `eop-T78923`→`T78923`, `ocds-e82gsb-566491`→`T566491`. **Join test on the 10 sampled authorities: SIGMA `unp` coverage rose from 5,133/7,830 (66%) via `unp` to 7,038/7,830 (90%) via `cais_id` — +1,905 contracts** (СОАПИ 58→94%, УМБАЛ Пловдив 33→86%, ЕСО 37→69%, АПИ 69→91%). `tsc` clean, prettier clean, round-trip test green (the generated column is outside the `COLUMNS` map, like `title_fold`, so lossless capture is unaffected). Applied to local Postgres; rides the next cloud reload (a STORED-column ALTER rewrites the table under AccessExclusive, so it must not be applied surgically to live Cloud SQL during serving hours — see [[reference_contracts_reload_lock]]).
