...
- **`useProcurementScope()`** (`src/data/procurement/useProcurementScope.ts`) — the segmented
  control + URL param `?pscope=ns|all|y:<year>` (default `ns` = the selected parliament's contract
  window, omitted from the URL; `all` = full corpus; `y:2024` = one calendar year).
...
- The host passes `scopeWindow={{ from, to }}` (a `ScopeWindow`) into the pack (`CompanyDbScreen`
  L913).
...
- Therefore, when the user narrows the scope, flag every non-following band with the **scope chip**:
  ...
  independent of scope" (NZOK `NzokPack` L93–101). Show it **only** when scope is actually narrowed
  (both bounds null under `all`); the exact period stays in each tile's footnote.
...
4. **Tax-gap band** (`nap-gap`): BG VAT gap **8.6% / €781M (2023)**, PIT gap **13.8%** vs EU 9.5%,
   ...
   modelled on `NzokSavingsLeaderboardTile` (the "if everyone paid the peer-median" civic question):
   "ако данъчната пропаст беше на нивото на медианата в ЕС, хазната щеше да събере +€X" — one
   number, one civic question. Consider a **peer-growth percentile strip** (`NzokPeerGrowthStrip`
   idiom) placing BG's gap in the EU distribution ("по-нисък от N% от страните в ЕС") — the
   transparent, non-black-box comparative.
