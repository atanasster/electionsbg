...
**Status**: Phases 0–2 implemented 2026-09-16 — the billed A/B is in §11, and Phase 3's file split is the one open item. **Rev 4**, 2026-09-16.
**Revision note**:
...
**Outcome**: §11 (2026-09-16) records what shipped, the three billed runs, the published-run manifest behind the evals page, and the open work. §1's starting-point figures are left exactly as measured, so the plan still reads as a before-and-after pair.
...
| **G1a** | **billed** | **Accuracy under narrowing — RUN 2026-09-16** | Two runs against the published baseline, compared on the case ids both contain. **The published baseline is NOT a like-for-like reference any more**, for two measured reasons: its argument scores predate a normalizer change (re-scoring its OWN saved outputs drops args from 98.1% to 79.2%/75.5% with tool accuracy unchanged), and its prompt is from 2026-09-10 while both new runs use today's. Both new configurations therefore show the SAME deltas against it (−0.8pt EN / −1.3pt BG tool), which is the signature of the vintage difference rather than of anything in this plan. **The informative comparison is the A/B between the two new runs**, identical except for the budget (`data/ai/evals/current_narrowed.json` with `EVAL_ROUTING_BUDGET=20000` → 62 tools kept, vs `current_production.json` → full catalogue): tool accuracy **EN 96.0% → 97.5% (+1.5pt), BG 96.0% → 96.2% (+0.2pt)**, arguments **81.1% → 88.7% / 75.5% → 81.1%**, registry group +1.2/+0.5pt, `challenge` equal, `realistic` EN +8.3pt and BG −4.2pt (**one case of 24**). So narrowing to 62 tools does not degrade tool choice on this sample; if anything it improves it, which is the direction §1.4's attention hypothesis predicted. Single stochastic passes at temperature 0, n=471 — not repeated trials. `gold-in-candidates` **99.3%** narrowed vs **100%** full, so the residual losses are model choices, not retrieval misses. Compare with `npx tsx ai/llm/evalCompare.ts <new.json> [reference.json]`. |
...
## 11. Implementation outcome (2026-09-16)

Phases 0 and 1 are implemented, and so is Phase 2 (C8). Phase 3's *contract* shipped without the file split (§11.4).

### 11.1 The billed runs — and what the A/B says

Three runs exist, each under its own label, and none overwrote an older artifact: `currentEval.run.ts` refuses to write a published file that already exists unless `EVAL_ALLOW_OVERWRITE=1` is set explicitly. That guard is the reason a group measurement could become a first-class artifact at all — the earlier rule refused to publish any non-full-suite run.

| artifact | cases/lang | budget | tool EN/BG | call EN/BG | args EN/BG | gold in candidates |
|---|---|---|---|---|---|---|
| `current_production.json` | 474 | 92,000 (real) | 96.0 / 96.0 | 85.9 / 86.9 | 81.1 / 75.5 | 948/948 (100%) |
| `current_narrowed.json` | 474 | **20,000 (forced)** | **97.5 / 96.2** | 89.2 / 95.1 | 88.7 / 81.1 | 941/948 (99.3%), mean **61.9** of 235 tools kept |
| `current_starter.json` | **367** | 92,000 | 82.6 / 81.7 | 57.8 / 67.3 | 42.1 / 56.0 | 734/734 |

The informative comparison is the two middle rows: same prompt, same normalizer, same vintage, differing only in the budget. **Narrowing to a mean of 62 tools does not degrade tool choice** — it is +1.5pt EN / +0.2pt BG on tool accuracy and +7.6 / +5.6pt on arguments — which is the direction §1.4's attention hypothesis predicted. `realistic` moves +8.3pt EN and −4.2pt BG (one case of 24). Both new runs sit −0.8pt EN / −1.3pt BG below the 2026-09-10 artifact *identically*, which is the signature of a prompt-vintage difference rather than of this plan; G1a carries the argument-normalizer evidence.

The starter row is the first measurement of the C7 bank, and it is deliberately harsher than the suite it is added to: 367 starter prompts score 82.6/81.7 tool against 96.0/96.0 on the curated 474, and call accuracy 57.8/67.3 against 85.9/86.9. **The starter bank is an ADDITIONAL, opt-in group, not a replacement suite** — the published floors stay registered on the 474-case corpus, and the page shows both. Two things read as failures and are not: the low `call` rate is dominated by cases whose registry entry declares an expected tool but no arguments (the suite cannot confirm a usable call without them), and 50 EN / 52 BG of the 242/241 argument-annotated cases were satisfied by deterministic entity extraction rather than by the model.

### 11.2 One manifest, and the page reads it

`data/ai/evals/index.json` is generated by [`evalsIndex.ts`](../../ai/llm/evalsIndex.ts) from every `current_*.json` on disk. The page ([`EvalsScreen.tsx`](../../ai/app/EvalsScreen.tsx)) fetches **that file** rather than hardcoding filenames, so a new run appears with no page edit and a renamed artifact cannot leave a dead link; `evalsIndex.test.ts` pins coverage-and-no-orphans, filename identity and exact regeneration. Two sections were added: **All published runs** (every artifact, oldest kept, with its budget, forced-budget flag, mean candidates kept and gold-in-candidates ceiling) and **Deterministic router (no AI)** (the free lane, on both the 841 cases and the 474 the floors were registered on). `EvalsScreen.test.tsx` renders the page against the committed artifacts through the real `fetchData` seam.

⚠️ **The page is only as live as the bucket.** It fetches `/ai/evals/index.json` through `fetchData`, so a `data/**`-to-GCS sync is required for a new run to appear on electionsbg.com; committing the artifact is not publishing it.

### 11.3 Suite state at this revision

`ai/` is 130 files / 5,269 tests green; the deterministic lane is 2,167 of 2,168 (`ai:test`), the one failure pre-existing and data-level (Габрово registered population, §11.4); `npx tsc -b`, `npm run typecheck:ai` and `npx eslint .` are clean and `npm run build:ai` builds.

### 11.4 Open work, recorded so it is not mistaken for done

- **Phase 3 did not happen.** The 4,011-line `routeText` was not split; what shipped is the precedence contract and the no-two-routers-disagree conflict test, which are the parts a split would have needed anyway.
- **The one pre-existing deterministic failure** — `"Какво е регистрираното население на Габрово?"` returns `57 582` against an expected `57 742`.
- **The starter group was not added to the *narrowed* configuration**, so the billed A/B has no argument-bearing corpus at budget.
- **`ClarifyDialog` has no render test** (skip-link / focus behaviour).
