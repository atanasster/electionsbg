...
### v1.2 → as-built (2026-09-09, Tier 1/Tier 2 implementation)

| #   | as-planned                                                                | as-built, and why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --- | ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 42  | Every site lister's `isElectoral` title rule was assumed correct as documented | Measured live 2026-09-09: Trend's own listing carries a post titled „Профил на избирателя (екзит пол от изборите за Народно събрание през април 2026)" — a post-election voter-profile retrospective — which the bare `"избори"` substring in its `ELECTORAL_TERMS` false-matched via "изборите". Decision 17's exit-poll exclusion (previously only on Sova Harris) is now shared (`wp_lister.ts`'s `EXIT_POLL_TERMS`/`isExitPollTitle`, used by `titleContainsAny` and therefore by Trend/Global Metrics/Gallup/Sova Harris uniformly) and applied to Alpha Research's differently-shaped rule too, which had the same exposure via an even broader `"избор"` substring. |
| 43  | `polls:fetch` fetches every discovered PDF via one `Promise.all`           | Measured live: Market Links' `/bg/news/storage/*.pdf` answers every fetch (curl, Node, with/without Referer or browser headers) with a bare WAF 403, while the linking page fetches fine. The original all-or-nothing `Promise.all` lost the page HTML too. Each attachment now fetches independently (`fetchAttachments`); a failure is logged and recorded in `SOURCE.json.attachmentFailures` rather than losing the capture. `fetchBinary`'s retry (added in the same pass) also had to be fixed twice: the first cut retried every failure once, including a permanent 403 and a genuine timeout, because the "permanent, fail now" throw was still inside the same try/catch its own retry-once catch guarded — narrowing the try to just the `fetch()` call itself (not the status check) was what actually fixed it. |
| 44  | §6.2's AR row: "`#content` text only, spam tokens stripped" → sentence rule everywhere | Measured on all 4 of AR's real captures gathered this session: 2 of 4 (full narrative posts) DO carry every party share inline in `#content` text, sentence-rule-extractable exactly as documented — but the other 2 (short "campaign start/end" posts) carry ONLY the passport paragraph in text; every party share is rendered solely inside `GraphN.jpg` chart images the lister/fetcher does not even discover today. Left as ⚑ §11.11 — extending the Gemini Vision fallback (§6.2, currently scoped to Sova Harris alone) to AR's chart-only posts is a real design decision (cost/complexity vs. losing ~half of AR's history to `refused` drafts), not a default to guess at. |
| 45  | Global Metrics' July 2026 capture would exercise the Tier 2 parliamentary aligned-row extractor | Confirmed by measurement (`pdftotext -layout` on the real captured PDF) that it is genuinely presidential — "Ако президентските избори бяха следващата неделя за кандидат президент... Кандидат на Прогресивна България 39.7%..." — consistent with decision 2/§7's own framing, not a new problem. Recorded because it means GM's only real capture to date cannot double as a Tier 2 (parliamentary) fixture; a parliamentary aligned-row fixture still needs either a future GM parliamentary post or Market Links' PDF (currently WAF-blocked, #43). |
