...
competition / HHI procurement tiles. Redundancy fix landed (НОИ reclaimed its NoiPack). **Infra follow-ups also SHIPPED
2026-07-17:** AI `socialSpending`/`socialBenefits`/`socialPovertyImpact` chat tools (864/864 ai:test pass,
pensions win the router overlap), the `asp_benefits` watcher + `update-social` skill + process-watch mapping,
`data_map/model.ts` wiring (social SOURCE_GROUP/DATASET/FEATURE + `/^\/social\//` AI_PATH_RULE), and the
README sector bullet + `/data/sources` entries. The **ГИТ labour-inspection tile** (§4.10) also shipped —
`SocialInspectionTile` (49,660 inspections → 189,576 violations, 2025) + curated
`data/social/git_inspections.json` (pdftotext-verified) + `git_inspections` watcher + `update-social` §3.
Mobile + dark-mode verified at 375px (0px overflow, all custom tiles legible). **Everything in this plan is
now built** — the only build-time auto-step is the OG image (`scripts/og/screenshot_sectors.ts` bulk loop).
Original plan/audit preserved below.
