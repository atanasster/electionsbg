...
README sector bullet + `/data/sources` entries. **Only remaining:** the OG-image regen (auto-generated by
the `scripts/og/screenshot_sectors.ts` bulk loop — runs at build) and the Phase-3+ ГИТ inspection tile.
