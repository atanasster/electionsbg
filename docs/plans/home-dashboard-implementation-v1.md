...
**Done 2026-09-01.** Operator runbook: [`docs/home-dashboard-operations.md`](../home-dashboard-operations.md).

- **One generation step, at the END of the watcher run** — `process-watch-report`'s **Final post-step 7b**, run unconditionally and once. Nine families feed the feed, so a per-source mapping would run the generators up to nine times in one run, each fold catching whichever siblings had not yet finished. ⚠️ It must be a numbered step in `## Procedure`, not prose in the reference region: the first cut sat ~140 lines above it, so an orchestrator following the numbered steps never built the entry page's artifacts while a `runbook.includes(…)` gate over the whole file passed.
- ⚠️ **THE ORCHESTRATOR DOES NOT PUBLISH — it records `home` in `state/upload/pending.json`** and hands off to `/upload-watch-changes`, like every other subtree. `npm run home:publish` (`bucket:sync:paths -- home && bucket:gz`) is the operator's one-liner. The gzip half is not optional: `gsutil rsync -j json` sets a TRANSPORT encoding only, so a sync reverts the stored gzip on the entry page's critical path — and `bucket:gz` did not cover `home/**` at all until review found it, so all four documents were giving a rationale for a command that selected none of these files.
- **`npm run home:health`** has TWO freshness arms, and neither substitutes for the other:
  - **absolute** (`unbuilt`) — the newest OBSERVATION any family reported, against the wall clock. ⚠️ Measured on `observedAt`, never on `computedAt`: both artifacts date themselves by their newest source vintage, so `hub_stats` sitting weeks back is its ordinary state (quarterly macro, sibling hub blobs) and flagging it is a false positive on every run — which the first cut did.
  - **relative** (`stale`) — each family against its declared cadence (`STALE_AFTER_DAYS`), computed LIVE from the code rather than from the flag frozen in the artifact, so tuning a ceiling changes the verdict and not just the printed number.
  The relative arm alone reproduces exactly the blindness this tool is justified by attributing to `db:check-generated`: `stale` is measured inside one artifact, so when the whole pipeline stops every lag stays put and it prints „healthy" for ever. `--public` fetches the two objects a browser reads, compares their content and checks `x-goog-stored-content-encoding` — the header is the only thing that can see a missing `bucket:gz`. It exits non-zero on missing / corrupt / schema / unbuilt / unavailable / stale / unpublished / drifted.
- ⚠️ **A family with no cadence is reported and never flagged.** `elections` is `null` because Bulgaria's last two are **539 days** apart; a ceiling loose enough for that detects nothing, so the honest answer is that the family has no cadence at all. `intl_debt` is `null` because it publishes nothing.
- **The bucket has ONE definition**, `scripts/db/lib/bucket.ts`, imported by four scripts. It was restated five times and the fifth copy — the one that FETCHES — named a bucket that does not exist, so `--public` reported both artifacts unpublished on every run and its `drifted` branch was unreachable. A smoke test could not have caught it: the correct bucket 404s too while the objects are unpublished.
- **`scripts/db/gen_home/runbook.test.ts`** keeps the prose, the registry and the publish in step: every `REFRESH_GENERATORS` home artifact must be inside `## Procedure`; `home:publish` must be the exact string (a substring test accepted `bucket:gz:dry`); `bucket:gz`'s own set must COVER the objects the page reads; and every adapter must have a `STALE_AFTER_DAYS` entry, keyed to the adapter-id union so a missing one is a compile error rather than a family `home:health` silently skips. Both directions asserted, with a non-vacuity floor on the derivation.
- **Atomicity and refusal were already in place from Phases 2/4/5** and are now documented as the contract: all three generators build in memory, validate, write a temporary sibling, rename; all three refuse rather than write an empty artifact.
