...
from both ends. The former publication gap is now closed by `proc:verify-seats:cloud`, wired as
the final check in the procurement update skill; the shared comparator also refuses the vacuous
empty-on-both-sides case.
