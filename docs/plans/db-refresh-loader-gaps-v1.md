...
|---|---|---|---|
| 1 | `db:load:nzok-hospital:pg` | first of the nzok block | reads `hospital_eik` + `hospital_ownership` + `hospital_payments` (all tracked); writes `nzok_hospital_payments`, which 2 and §1c's financials loader read |
| 2 | `db:load:nzok-hospital-map:pg` | after 1 | reads **no JSON at all** — derives geo from `nzok_hospital_payments` (`load_nzok_hospital_map_pg.ts:78`) |
