...
**Not started:** T1.1 (largely pre-existing), T2.0–T2.3, T3.1–T3.3, T4.0,
...
**T2.0 — label articles into events before changing the rule.** The original “92 raw
...
**T2.1 — a second candidate and join channel.** Extend candidate retrieval as well as
...
**T2.2 — proposals, review and stable identity.** Keep the conservative auto-accept rule
...
**T2.3 — release on accuracy, monitor cluster shape.** ⚠️ Two in-repo priors bear directly
...
**T3.1 — taxonomy v2: `elections-presidential`.** A category of its own, not a subcategory
...
**T3.2 — review and reclassify the candidate set.** The 360 keyword matches are a
...
**T3.3 — cases (казуси), not a "scandals" category.** „Петрохан" and „помилването" are not
...
**T4.0 — news-person identity before person sentiment.** Create a versioned
...
**T5.1 — lead with the event and evidenced differences.** Above the fold: event title,
...
**T5.2 — port `StoryCard`'s divergence guard to `StoryScreen`.** Use an explicit state
...
**T5.3 — stop hiding `not_applicable`.** Either draw it as its own muted segment or print
...
**T5.4 — one completeness sentence, once.** Replace the five-figure strip on the story screen with
...
**T5.5 — make the timeline a timeline.** A dated rail, outlet name and mark, the framing
...
**T5.6 — de-jargon, one pass, both languages.** „клъстер" → „група материали" and
...
It is not a story-page string (it exists only in `TopicsScreen.tsx:300,352,442`). On 2026-09-21, commit `6289ba8e96`
...
**T5.7 — a filtered-empty timeline needs a way out.** An explicit „Покажи всички" when a
...
**T5.8 — make comparison an action.** Select two or three source headlines; align outlet,
