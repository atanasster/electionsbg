...
| **T4.2** — publish party treatment | ✅ | `git log --grep T4.2` (2026-09-22) | `news/scripts/party_rollups.py` folds the PUBLISHED party tones (T4.1's gate withheld the rest) into dated count distributions with visible denominators: `parties.json` (per party — counts, `assessed`, articles, outlets, window, `undated`) and a PAGINATED `party/<id>.json` + `party/<id>-<n>.json` (50 rows a page; the largest party went 147 KB / 33.5 KB gz unbounded → 43 KB / 11 KB gz), each row carrying the tone, the model's rationale and the LOCATED quotes — short evidence only. Topic rollups ride `taxonomy.json` as `by_party_id_tone`, named for its key space because the story-level `by_party_tone` is keyed by the NAME as written. REFUSALS, each mutation-checked: no resolved `party_id` → no row and no page (a name is not an identity), and the bucket SPLITS into „the registry knows this surface and refuses it“ vs „the registry does not cover this party at all“ (mostly foreign), both printed; an id the path charset refuses is dropped ONCE at the producer, so index, payloads and links cannot disagree (`partyId.ts` ↔ `party_rollups.PARTY_ID_SAFE`, pinned by a test that reads the Python source); one pair per (article, party), like both story writers, with a second spelling counted as `duplicate_pairs`; order is COVERAGE, never favourability, and nothing carries a score; T4.1c scoped observations excluded and counted; rows sort on the INSTANT, not the string; a payload whose party loses its last tone is PRUNED. `/party/:id` is strictly the plan's ARCHIVE FILTER and says so on the page; `/parties` is its index. ⚠️ Route divergence, flagged as the plan asks: the sibling plan says v1 needs no new top-level route — this keeps the archive route because the evidence contract now makes the rows real, and constrains it to an archive rather than a scoreboard. Both are registered in `newsapp/App.tsx`, `prerenderRoutes.ts` (news origin, no-slash, sitemapped only where `assessed > 0`) and the nav; `PartyScreen` guards the charset BEFORE the error branch (an unserveable id must not load for ever) and paginates. Measured 2026-09-22: 1,720 published tones → 17 parties / 488 pairs; 979 pairs on unresolved surfaces, 278 scoped out, both printed on `/parties`. Overlay vectors regenerated. 14 Python, 10 vitest, mutation-checked (favourability order; rollup rule dropped; unresolved rolled up; unsafe id kept; dedup dropped; stale payload kept; guard order; client link guard). |
| **T4.3** — generic person treatment | ✅ | `git log --grep T4.3` (2026-09-22) | `news/scripts/person_tones.py` is the stored contract, the validator, the gate and the generator. A row is exactly the plan's shape (`news_person_id`, `mention_refs[]`, `subject_role`, `assessment_status`, `tone`, `confidence`, `rationale`, `evidence_spans[]`, `quoted_attitudes[]`, `text_scope`, `model_version`, `rubric_version` (`person-treatment-v1`), `identity_version`, `assessed_at`). FOUR REFUSALS, each mutation-checked: (1) no identity, no public tone — targets come from the T4.0 resolver's RESOLVED set for that article, and an ambiguous surface stays in the `news_persons` sidecar with its basis, never carrying a tone (`ambiguous_identity` is a property of the mention record, not of a tone); (2) a directional claim needs a LOCATED quote (the axes' and party tones' own `locate_evidence_spans` / `party_tone_spans_support`, reused not reimplemented) — an unsupported claim about a NAMED PERSON becomes `not_assessed`, never a published tone; (3) an incidental mention gets no forced sentiment; (4) a partial read is `insufficient_text` (T4.1c), never an article-wide claim. The validator enforces status↔tone consistency (`assessed` requires a tone; everything else forbids one), ids against the article's candidate set, duplicate (article, person, rubric) pairs, and every provenance field. The cache key is rubric + article content + every target's `identity_version`, so a re-slug, a merge or an edit detaches the stored tone rather than ageing it. `ArticleScreen` renders the treatment beside the person's row: role, „представен: <tone>“ with the rationale and each located quote (voice and speaker named; an unlocated one struck through), and the STATUS in words wherever there is no tone — so „no judgement“ can never read as „neutral“ — with the caption stating that an assessment is of HOW the article presents the person, never of the person, their conduct or guilt. Measured 2026-09-22 on a full regeneration under the repaired contract (GLM-5.3-flash over the 53 articles carrying a resolved identity): 53 ok / 0 failed; 26 assessed (8 primary, 18 secondary), 13 `insufficient_text`, 10 incidental `not_assessed`, 3 refused for unsupported evidence; tones 16 unfavorable / 8 neutral / 2 favorable; 10 model claims dropped (6 out-of-range targets, 3 unsupported, 1 failing its own contract — dropped PER ROW, so one bad row never discards another person's evidenced one). ⚠️ The registry holds ONE active identity today (T4.0), so that is the whole assessable corpus — the pilots' remaining scenarios wait on reviewed identities, not on code. 21 Python, 1 vitest. |
| **T4.4** — complete the news-person publishing path | ✅ | `git log --grep T4.4` (2026-09-22) | `news/scripts/person_rollups.py` + `write_person_shards` turn T4.0 identities and T4.3 treatments into one shard per ACTIVE identity (`news/app-data/person/<id>.json`) plus coverage on every `news_persons.json` row, and `/person/:newsPersonId` renders it. ⚠️ THE DENOMINATOR IS THE FEATURE, not a percentage: **M** = eligible (person, article) pairs — publishable articles carrying a resolved PRIMARY or SECONDARY mention; **N** = the subset with a valid FULL-TEXT assessment under the displayed rubric; and two identities are enforced in code and asserted before anything is written — `sum(tone counts) == N` and `N + insufficient_text + pending + refused == M`, the three unassessed kinds being mutually exclusive. A shard whose accounting fails is REFUSED and named, never published, because an accounting a reader cannot check is worse than no page. Everything outside M is reported BESIDE it: incidental mentions (named in passing — they create no eligibility and, per the plan, no page), unresolved mentions (the registry declined to say it was them), and `analyzed_count`, from which nothing here derives N. Syndication is named for what the corpus can show — `same_headline_copies` / `eligible_deduplicated`, published beside the raw denominator rather than replacing it, and folded per HEADLINE across DIFFERENT outlets so one outlet republishing itself cannot deflate M. `per_outlet` carries each outlet's own distribution, never one inferred tone per outlet. A page is a HUMAN decision: only an identity the registry marks `active` gets one, a deactivated identity's shard is PRUNED from disk, an id outside `NEWS_PERSON_ID_SAFE` (`[a-z0-9_]{1,64}`) is refused on both sides, and a main-site bridge is rendered only from `verified_main_site_slug` — a null one is an explicit sentence, never a guessed link. Rows carry the SHORT located quotes and provenance only (offsets stripped), each row's own status in words wherever there is no tone, and `docs/policies/news-person-pages.md` (`news-person-policy-v1`) states the rules — who gets a page, no minors, no contact data for disambiguation, deactivation removes the shard, correction and right-of-reply paths, a named review owner — surfaced on `/methodology` („Страници за лица") in both languages, not only in the repo. The correction link's path allowlist gained a person arm carrying that same charset; before it the person page's „Сигнал за поправка" silently dropped the path and asked the reader to type the address. Measured 2026-09-22 on the live build: 1 active identity with coverage, **M = 43** (42 deduplicated; 1 same headline across 2 outlets), **N = 26** (16 unfavorable / 8 neutral / 2 favorable), 13 `insufficient_text`, 1 pending, 3 refused, 10 incidental outside M, 22 outlets, 38 stories, 53 rows rendered, 99,194-byte shard — larger than T4.2's paginated party page, so the same `PARTY_PAGE_SIZE` treatment is the shape to apply when a second identity is activated. A tone is published ONLY beside an `assessed` status and only on a row inside M, refused on BOTH sides — an incidental mention carrying a stray tone passes the accounting (it is outside M), so without that refusal it would render a negative framing label about a named person that appears in no count and rests on no evidence. A row whose `subject_role` is unreadable is counted as `unreadable_role` OUTSIDE M rather than inside it: counting it in would break the second identity by construction and cost that person their whole page over one malformed row. `partial_scope` is published as a SPLIT of `insufficient_text` (13 of 13 today), `undated` counts the 7 rows inside M that carry no date so the window beside them cannot read as covering everything, and ordering is on the INSTANT via the shared `rollup_common.py` — the corpus carries both `+00:00` and `Z`, which string-sort wrongly, a lesson `party_rollups` had already learned. The reviewer comes from the REGISTRY, not from `public_index`, which exports `reviewed_at` and no owner — every shard published a null one until the review caught it. The id charset now lives once, in `newsapp/app/newsPersonId.ts`, with `newsPersonId.test.ts` pinning it to the Python twin and `corrections.ts` COMPOSING it instead of restating it. The archive paginates at 50 rows (the party precedent), the accounting above the fold is over ALL pages and the pager says so, and the resolved name on `ArticleScreen` is the ONE inbound link — without it the family is reachable only from the sitemap, which is what would make a routing bug there invisible to manual testing. Measured 2026-09-22 on the live build: 1 active identity, **M = 43** (42 deduplicated; 1 same headline across 2 outlets), **N = 26** (16 unfavorable / 8 neutral / 2 favorable), 13 `insufficient_text` (all 13 partial-scope), 1 pending, 3 refused, 10 incidental and 0 unreadable-role outside M, 7 undated, 22 outlets, 38 stories, 53 rows over 2 pages, 94,524-byte first page. Gates: `test_person_rollups.py` (15; mutation-checked — folding incidental into M, one inferred tone per outlet, a tone beside a non-assessed status, an unreadable role inside M, and a reviewer read from the public index, each fail), `NewsPersonScreen.test.tsx` (6), `newsPersonId.test.ts` (3), `corrections.test.ts` (2), `prerender.test.ts` (+1), `ArticleScreen.test.tsx` (+1), `MethodologyScreen.test.tsx` (+1). ⚠️ **SHIPPED IS THE PRODUCER AND THE NEWS-SIDE PAGE ONLY — three halves of the spec are NOT built**: (1) the main-site consumer — `src/` has no news-mention hook, no „in the news" renderer and no types for one, so the „extend the existing person mention shards compatibly" paragraph is untouched; (2) the story and case views do not carry person counts, only the person view does; (3) `MAX_TARGETS`-capped pairs file as `pending` („not yet assessed"), a claim that will never become true for them, since `targets_dropped` does not reach this rollup. ⚠️ One active identity is the whole publishable set; the 1,566 queued surfaces wait on a reviewer, not on code. |
| **T4.5** — person release gates | ⚠️ **BUILT AND UNMET — 0 adjudicated pairs** | `git log --grep T4.5` (2026-09-22) | `news/scripts/person_accuracy_gate.py` + `news/evals/person_adjudications.json` + `news/scripts/sample_person_pairs.py` (`npm run news:person:gate` / `news:person:gate:enforce` / `news:person:sample`). The floors live beside the party ones in `score_analyses.py` (`PERSON_RELEASE_GATES`) and are deliberately the SAME numbers — pair precision ≥0.95, pair recall ≥0.90, tone macro-F1 ≥0.80, per-tone recall ≥0.70, zero wrong canonical targets, zero unsupported evidence. ⚠️⚠️ **THE GATE IS UNMET AND SAYS SO: the adjudication file holds ZERO pairs** — the public eval surface has never received a submission, so every floor is evaluated at n = 0, `--enforce` exits 1, and the status is „UNMET — 0 adjudicated pairs; nothing here is a measured accuracy". This ships the arithmetic so the day a human writes pairs the answer is computed rather than argued; it does not fill the file, and a frontier model may not fill it either. **FOUR WAYS IT COULD HAVE REPORTED A GREEN GATE WITHOUT SUPPORT, all found by review and all closed**: (1) the two release blockers were counted as „is it True" / „is it False", so the `null` the sampler emits read as „not wrong" and „supported" — 200 pairs whose blockers nobody examined reported **MET**; an unadjudicated blocker field is now an EXCLUSION, which drops the sample below the support floor; (2) the pair key omitted the SURFACE, so every unresolved person in one article collapsed into one pair — 17 of 250 real candidates silently dropped, concentrated in exactly the population a recall floor is about, and fed to Cohen's κ as cross-person „agreement"; (3) the rubric filter was OFF by default and the identity filter had no CLI flag at all, so 200 pairs judged under a superseded rubric reported MET with `excluded: {}` — both are now on by default, the identity versions read from the published corpus; (4) κ was computed, printed and never gated, so a 200-pair sample labelled start to finish by ONE annotator passed everything — it is now part of support (≥40 doubly-annotated pairs, κ ≥ 0.60). A fifth, in the opposite direction, made the gate **unmeetable**: the producer is validated never to emit a tone beside a non-assessed status, so a pipeline in perfect agreement with the human scored 0.0 recall on every `not_assessed` pair — half the live corpus — and since per-tone recall is a `min`, one such label failed the gate however good the pipeline was; the pipeline's STATUS is now carried and scored as the answer it is, with `insufficient_text` kept distinct from `not_assessed`. **SUPPORT IS PART OF THE GATE** (≥200 test pairs, ≥30 per tone, all five strata non-empty, the agreement floors): without it a three-pair sample at 1.000 precision „passes", which is the vacuous pass §15 forbids — insufficient support is REVIEW-ONLY. **A zero over an empty sample is not a pass either**: the two maximum checks fail closed at n = 0, and even a MET gate prints the 95% upper bound its zeros can support — **1.88% (Wilson) at n = 200**, not the ~1.5% rule-of-three approximation, because the tighter figure under-states the uncertainty. Also computed: Wilson bounds and an ARTICLE-bootstrap interval that reports its seed, rounds and discarded resamples (and returns no interval when most were undefined), resolved-target coverage, a `role_mismatches` count (a human disagreeing with the pipeline's `subject_role` is stating the thing that would move T4.4's M), natural vs hard-case prevalence reported SEPARATELY, and the published neutral/unfavourable shares as a drift baseline — folded ONE PER IDENTITY, page 1 only, since T4.4 writes the whole-identity accounting onto every page and summing the shards reported `assessed: 52` against a true 26. An `adjudicated: true` row WINS over either annotator, which is what the contract promises; without it the answer was whichever row came first in file order. The sampler draws CANDIDATES with every label field `null`, over the whole per-domain corpus rather than the 150-article `latest.json` window, and includes UNRESOLVED and ambiguous mentions — sampling only what the pipeline resolved would make recall unmeasurable by construction. Measured 2026-09-22 after the repairs: **250 candidates drawn against a 200 target, no pair shortfall**; available `generic_person` 15,673 · `long_article` 4,175 · **`official` 2,403** · `quotation` 2,236 · **`ambiguous_name` 0**; distinct SUBJECTS in the draw 142 / 94 / 45 / 65 / 0. ⚠️ The `official` stratum was **53 pairs over ONE person** until the review caught it — it read the 1-person news identity registry rather than the 5,121-person gazetteer its own docstring named, and unlike an honest zero it reported as healthy coverage; the draw now carries a distinct-subject count per stratum so „53 pairs over 1 person" can never render as „53 officials". `ambiguous_name` stays 0 because the registry holds one active identity and has refused no surface as ambiguous. ⚠️ The committed gold set cannot supply these pairs (`shortfall: {person_linked: {wanted: 40, got: 30}}`); 200 pairs with ≥30 per tone is NEW annotation. ⚠️ No chain runs the enforcing alias yet — `news:release:gate` runs neither this nor its sibling `news:join:gate`, deliberately, since both would block every build today. Gates: `test_person_accuracy_gate.py` (34; mutation-checked — null blocker fields, a key without the surface, a status not treated as an answer, an ungated κ, a dropped support floor, dropped declined rows and a zero passing at n = 0 each fail, and one test asserts the committed adjudication file is still empty), with hand-computed Wilson and κ anchors rather than a window any wrong implementation would fit inside. |
| **Stage C journey** — the required acceptance walk | ✅ walkable · ⚠️ **NOT complete** | `git log --grep "Stage C"` (2026-09-22) | `news/scripts/journey_gate.py` (`npm run news:journey:gate` / `:enforce`) walks the plan's own sentence over the BUILT corpus — briefing → the Петрохан case → a specific event → ≥2 distinct outlets → the person → each assessment's voice and evidence → the original → a shareable comparison — reporting per step what it required and what it FOUND, so a step a reader cannot take is stated rather than inferred from a green test. Measured 2026-09-22 (all eight steps ok): briefing 16 events over a **30-day** window; the case listed with its detail shard (132 articles / 112 events); **15 events qualify on member count and 8 are walkable end to end**; the chosen one carries 3 members over 3 distinct domains; the active identity is on THAT event (1 row there, 52 across the case) and its page is read across **both** of its pages; **18 of 18** directional assessments carry a located, voiced quote beside 8 neutral rows; **53 of 53** rows link to the publisher. Three rules are load-bearing, and the first two were wrong in the first cut. **The chosen event must satisfy the WHOLE chain**: picking the first event with enough members chose one carrying ZERO of the person's rows, then checked the person against the whole CASE and reported `ok` along a path no reader could walk — while 8 walkable events sat unreported. **Every page of the person shard is read**: the shard paginates at 50 rows, so „every directional assessment" and „each row links out" were universal claims evaluated over 50 of 53 rows, printing `assessed: 26` three lines from a count of 24. And **a neutral row needs no quote** while a directional one needs a LOCATED one — „an absence is not provable by a quote" (T4.1b/T4.3), and an unlocated span is what the producer itself renders struck through. ⚠️⚠️ **WALKABLE IS NOT COMPLETE.** The plan also requires the journey „with a non-political person and an ambiguous namesake". Both variants now WALK their subject rather than asserting they could — so `ok` is reachable and the field means something — and both report `unavailable` today with a derived reason: the registry holds ONE active identity with coverage, and `news_person_candidates.json` publishes `ambiguous: 0`, read from the corpus rather than hardcoded so the claim cannot go stale after the first refusal. A variant that is not `ok` must say why, asserted in code, because the first cut flipped to a note-less `BLOCK` the moment a second identity appeared. That backlog is T4.0/T4.5's (1,802 queued surfaces), not a code gap, so `--enforce` guards the STEPS and an unavailable variant never fails a build — while `stage_c_complete` stays **false**. „Independently identified" is read narrowly as ≥2 distinct DOMAINS; this corpus carries no ownership data and the gate claims none, and whether today's briefing happens to carry one of the case's events is REPORTED (`reached_from_briefing: false`) rather than required. A malformed artifact is named rather than read as absent. Gates: `test_journey_gate.py` (15; mutation-checked — reading page 1 only, letting the chosen event not carry the person, ignoring `located`, and hardcoding the ambiguous count, each fail), fails closed on an empty corpus with every step blocked. |
| **Stage C corrections** — a correction rebuilds every consumer | ✅ gated (news side) · ⚠️ NOT atomic, and the main-site consumer is unbuilt | `git log --grep "Stage C"` (2026-09-22) | `test_build_app_data.py::CorrectionRebuildsEveryConsumer` (2 tests, 3 subprocess builds) pins the news half of „Correction/removal rebuilds story, case and news-person shards in one data transaction … A main-site bridge must not retain a removed tone." It walks the real correction: the identity is `active`, the build writes its page AND a stored, validator-clean treatment, the reviewer withdraws it, and the next build must leave **no index row, no shard on disk, and no id or tone on ANY published surface** — the per-domain bundle, `latest.json`, `stories.json`, `home.json`, `news_persons.json` and every story shard are each searched for both. ⚠️ **NON-VACUITY ON BOTH HALVES.** The first cut asserted „no tone afterwards" against a fixture that never had one — `[] == []` in both states, satisfied by the fixture rather than by the code — so the page AND the tone are now asserted to EXIST before the withdrawal, and the treatment is written against the `identity_version` the build itself derived, since a stale one silently yields no tone and puts the vacuity straight back. The withdrawal flips `status` and nothing else: a real one keeps the evidence trail, and flipping the alias too would leave the test green with the active-status guard deleted. Mutation-checked: deleting the shard-prune loop, and deleting `current_for`'s staleness guard so a stored document is served for an identity that no longer resolves, each fail it. ⚠️⚠️ **„THE BUILD IS THE TRANSACTION" IS NOT TRUE AS A DURABILITY CLAIM, and an earlier draft of this row said it was.** `write_json` writes in place and the prune `unlink()`s separately, so an INTERRUPTED run can leave exactly the mixed state the criterion forbids — a withdrawn identity's shard still on disk beside an index that no longer lists it. What the tests establish is that a COMPLETED build leaves every consumer consistent; a torn one is not covered by anything here. ⚠️ **The main-site consumer half of the same paragraph is NOT built** (flagged on T4.4): `src/` has no news-mention hook, no „in the news" renderer and no types for one, so „backwards-compatible fields, schema support checks and a coordinated correction/tombstone path" across that release boundary is open work. |

**Not started:** T1.1 (largely pre-existing), T6.x, T7.x.
⚠️ **T4.5 is BUILT and UNMET** — the gate exists and fails closed on an empty
adjudication file; it is not "not started" and it is not passing.
...
**T4.2 ✅ SHIPPED (§0.1) — publish party treatment.** Story `by_party_tone`, topic rollups and `/party/:id`
...
**T4.3 ✅ SHIPPED (§0.1) — generic person treatment.** Proposed stored contract, with article ID supplied by
the parent record:
...
**T4.4 ✅ SHIPPED (§0.1) — complete the publishing path.** Build news-person detail/index shards and a news
...
**T4.5 ⚠️ BUILT AND UNMET (§0.1) — release gates, with sufficient support.** Extend `score_analyses.py`’s existing
...
| C — named-subject treatment | ✅ built · ⚠️ NOT complete — see §0.1 T4.5 and the journey row | T4.0 identity + T4.1 evidence → T4.2 parties + T4.3 generic people → T4.4 integration | T4.5 gates (**built, UNMET — 0 adjudicated pairs**); the Kalushev/Petrohan journey **walks end to end**, but its two required variants have no subject; corrections rebuild every consumer ✅ gated on the news side — ⚠️ not atomic under an interrupted build, and the main-site consumer is unbuilt (T4.4) |
