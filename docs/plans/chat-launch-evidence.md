...
## Editorial revision: homepage chat card

The illustrated Попитай Наясно card replaces the slideshow beside search in the reviewable launch, with three starter links and How it works. The duplicate strip beneath search is removed. Unpublished production retains its existing slideshow; static home HTML follows the same publication/preview gate. BG/EN checks at 1280px, 390px and 320px find one invitation, no slideshow, no horizontal overflow and links at least 44 × 44px. The desktop header measures 454.625px against its unchanged 520px budget. Updated light desktop and dark mobile screenshots retain actual UI pixels. Seven focused prerender cases and scoped lint pass.

The complete preview build/postbuild passed. All nine selected HTML, entry, request-budget and slideshow-loading checks pass; the first-paint test now retries its exact-one locator assertion to wait for the lazy route mount. Source and repair review found no actionable issues. The refreshed hosted preview passes all six BG/EN desktop/390px/320px cases with one card, no slideshow and no overflow. Production publication remains gated.
