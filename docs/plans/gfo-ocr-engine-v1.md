...
garbage. The branch must test for *usable Cyrillic*, not for the presence of a font.
...
It *is* recoverable. Extracting the embedded JPEGs by scanning for `FFD8FF` markers and
...
| `9e7e6feb` | *(not a PDF)* | n/a | 🚫 | 🚫 | 🚫 400 | 🚫 400 | 🚫 400 |
...
- **Gemini 3.7-flash is 3.8× faster than 3.5-flash on the capture-everything task** (11.1 s
  vs 42.1 s per document) and *more* accurate. The gap is thinking tokens: 27,141 vs 134,918
...
- **v2's `код на реда` suppression collided with the revenue value range.** The rule "a
  ...
  form — and `12 098 хил.лв` of revenue is *also* in 10000–19999. On `5c954fae` the parser
...
- **The form detector was fooled by the amounts.** Detecting the coded form by counting
  5-digit numbers starting with `1` classifies the *uncoded* 2Р България form as coded,
...
- **3.5-flash loses a document to the harder task.** On `ab4488e9` — the worst scan in the
  ...
  real cost, and 3.5-flash pays it. It failed *safely* (a refusal, not a number), which is the
...
- **„Всичко (Общо приходи + Г)" is not total revenue.** On the coded form the bottom line
  adds the year's *loss* back to revenue, so for a loss-making company it exceeds `Общо
...
general contractor the analogous check is procurement receipts against revenue, which *is the
finding*. A real shell winner and an OCR misread produce the identical signal, so a gate on
...
identity C failed at *baseline* on 8/9 documents because of a bug in my own label regex, so
...
A shifted column is *internally consistent*: every identity still balances, because every term
...
form has one and fall back to a folded label otherwise**; a mismatch on a *headline* row is the
...
The three checks above are all *internal consistency*. None can catch a capture that is
...
withheld company-year can say *why*. This matters more here than in most of the repo: the
...
- **The reason for absence is stored, and the UI says which one.** Four distinguishable
  cases, all observed in this 16-document sample: *not filed yet* (the recency ramp — the
  honest label is „ГФО за 2025 г. още не е обявен"), *lawfully not published* (§1.4, чл. 38
  ал. 4 — „предприятието не публикува ОПР"), *filed but no ОПР in the documents we hold*
  (§1.3), and *read failed a consistency check* (§10.5). Only the first improves with time.
- **Every ratio names its year, and no ratio silently reaches back.** „Спечелил €X при оборот
  €Y **за 2023 г.**" — with the procurement € taken from the *same* year. Comparing 2025
...
1. **The Gemini list price.** No rate card was consulted and the repo records none. §4.2's
...
2. **The fetch rate and block behaviour of `/CR/api/Documents/{ActID}`.** Nothing was fetched.
...
5. **The real ОПР yield per EIK-year.** This sample gives 60% of documents and 1.59 documents
   per EIK-year, from 16 documents over 8 companies. The interaction — how often *some*
...
1. **Confirm the rate card** and replace §4.2's band with a number.
2. **Probe the fetch**: 200 documents against `/CR/api/Documents/{ActID}` via `curl`, measure
   throughput and block behaviour, and decide the crawl schedule. This is the real gate.
