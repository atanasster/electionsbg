...
```jsonc
{
  ...
  "figures": {                             // caption NUMBERS, never prose
    "procTotalEur": 93.9e9, "procContracts": 407392, "sofiaBuyerShare": 0.566,
    // ⚠️ `…ArcShare`, not `…Share`: these three are measured against
    // `flows.coverage.bothPlacedEur` (23.5% of the corpus) while `sofiaBuyerShare` two lines
    // above is measured against the buyer-placed layer (99.0%). They sit in one block and
    // their denominators are four times apart, so the name carries the basis.
    "sameOblastArcShare": 0.558, "intoSofiaArcShare": 0.311, "outOfSofiaArcShare": 0.073,
    "topFlow": ["PDV", "SOF", 1116],  // M€, like every other cell of the matrix
    // ⚠️ BOTH, always. The funds layer is ~48% of its corpus — 4.6% of ROWS carry no oblast
    // and they hold 52% of the MONEY — so a numerator without its denominator is a caption
    // that halves the corpus.
    "fundsPlacedEur": 16.06e9, "fundsTotalEur": 33.70e9, "agriTotalEur": 11.04e9
  }
}
```
...
- **A missing input drops its layer and records it in `available`**, the `hub_stats` idiom; it
  never zeroes a layer, because a zero column is a claim. ⚠️ `price_payloads` is the one that
  bites: `048_prices.sql` has NO applier anywhere in `db:refresh`, so on a clone that never
  ran the prices ingest the relation is simply absent, and an unguarded read would abort the
  chain five steps from the end — after the ~90-minute contracts and person work. It is
  preflighted inside `buildPrices`, not in the generator's `need` list, because a missing
  price corpus must drop the OVERLAY rather than skip the artifact.
- **The 28-oblast refusal applies to the ALL-TIME layer only.** Over a window days old — which
  the sitting-parliament scope is every time an election lands — an oblast that has signed no
  contract yet is a measurement, not a hole, and refusing there would abort `db:refresh` for
  being right. An unresolvable NAME is refused in both, because that one can move money
  between oblasts.
