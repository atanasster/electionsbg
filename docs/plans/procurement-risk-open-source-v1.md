...
| **T4** ✅ | `/procurement/methodology` (BG + EN, prerendered, both sitemap lists, own og:image), anchors for all 17 checks, the `risk-flags.json` download | T2, T3 | The page reads the SERVED version (T1.5), never the bundle's. `ogAndSitemapCoverage.test.ts` enforces the three artifacts — and caught the missing `<loc>`, since `npm run sitemap` is manual and its output committed. **Scope actually shipped:** framing, both flag tables with thresholds, the contract grade, both exposure weight sets, the alignment summary and the limits — i.e. §4a/§4b/§4d/§4f/§4g. **NOT on the page:** the legal-threshold detail (§4c) and the reconciliation rules (§4e), which stay in the handbook; and the chip anchors are contract-grain only — tender and exposure chips are not yet linked, though their anchor targets exist |
| **T5** ⏸ | (optional) publish the two scorers as a standalone subpackage with README | T1 | **Not triggered.** Its own condition is "only if SIGMA/others express interest", and nobody has asked. `risk-flags.json` plus the MIT grant already cover reuse without a package to maintain |
