...
> **Wage build (2026-06-09, final):** went to build the wage ingest. Found the clean path then lost it: NSI's static labour timeseries XLSX **are** fetchable (`Labour_2.1.2.xlsx` = employees by oblast, 2020-2024, parses cleanly with `xlsx`), but the **wage**-by-oblast file (`Labour_2.3.1`) the search indexes **no longer exists** — `old.nsi.bg` 301-redirects to the www homepage, and www keeps only the employees file, not wages. With the press-release table chart-locked and the content/Infostat pages Cloudflare-walled, **all 28 oblast wages now live only in the Infostat SPA.** Reaching it needs a **headed-browser** Playwright scrape (`cik_fetch.ts` pattern uses `headless:false` because headless triggers per-URL Turnstile) — which needs a **display**, so it's an **operator-run** ingest (like the CIK local-elections process), not runnable from a headless agent. Playwright + chromium + the cf_clearance-warming bypass are all installed. So the wage ingest can be *written* but only the operator can *run* it. Until then the affordability index stays on the GDP-per-capita proxy. **`Labour_2.1.2.xlsx` (employees by oblast) is a clean, fetchable bonus** if a per-oblast employment metric is ever wanted.
