// Framework-agnostic core of the composite "Индекс на изборния риск".
//
// This is the single source of truth for the 0–100 headline + the 10 component
// sub-scores. It takes already-resolved data objects (no React, no fetching) so
// BOTH consumers can share it:
//   • the React hook `useRiskComposite` (passes its React-Query results), and
//   • the AI-chat `riskIndex` tool (fetches the same JSON via the tools layer).
//
// Keep this file pure (no `@/` runtime imports, no hooks) — the AI tools layer
// imports it under tsx/node where those don't resolve. Type-only imports are
// erased, so referencing the hook-file types here is safe.

import type { Votes, ElectionInfo } from "../dataTypes";
import type { RiskScoreSummary } from "./useRiskScore";
import type { RiskClustersReport } from "./useRiskClusters";
import type { SuspiciousSettlementsReport } from "../dashboard/useSuspiciousSections";
import type { BenfordReport } from "../benford/useBenford";
import type { NationalSummary } from "../dashboard/dashboardTypes";
import type { ProblemSectionsReport } from "../reports/useProblemSections";
import type { PollsAccuracy } from "../polls/pollsTypes";
...
// All data the composite needs, already resolved by the caller. Each field
// mirrors one React-Query hook the `useRiskComposite` hook reads; the AI tool
// fetches the same JSON and aggregates `countryVotes` itself.
export type RiskCompositeInputs = {
  /** Selected election ("YYYY_MM_DD") — the coherence anchor. */
  selected: string;
  risk: RiskScoreSummary | null | undefined;
  /** Country-aggregated vote rows (machineVotes/suemgVotes summed PER party
   *  across regions); null when region votes are unavailable. */
  countryVotes: { results: { votes: Votes[] } } | null | undefined;
  suspicious: SuspiciousSettlementsReport | null | undefined;
  benford: BenfordReport | null | undefined;
  national: NationalSummary | null | undefined;
  problemSections: ProblemSectionsReport | null | undefined;
  problemSectionsStats: ElectionInfo[] | null | undefined;
  pollsAccuracy: PollsAccuracy | null | undefined;
  clusters: RiskClustersReport | null | undefined;
  electionStats: ElectionInfo | undefined;
  priorElections: ElectionInfo | undefined;
};

export const BAND = (score: number): RiskCompositeBand =>
  score < 20
    ? "calm"
    : score < 40
      ? "elevated"
...
// Pure composite computation. Returns null when the inputs are incoherent
// (still loading, or mid-transition between elections) or when no integrity
// component is available. The caller decides what to do with null (the hook
// keeps the last coherent value; the tool emits an "unavailable" envelope).
export const computeRiskComposite = (
  inputs: RiskCompositeInputs,
): RiskComposite | null => {
  const {
    selected,
    risk,
  ...
  if (countryVotes) {
    ...
    let totalDrift = 0;
    let hasFlash = false;
    for (const v of countryVotes.results.votes) {
  ...
  if (problemSections?.neighborhoods?.length && problemSectionsStats) {
    ...
    const topMeta = topPartyNum != null ? currentMahala.get(topPartyNum) : null;
    ...
    if (
      ...
      const value = Math.min(100, (100 * excessPp) / NEIGHBORHOOD_SWING_CAP_PP);
