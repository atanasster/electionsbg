...
// Procedure code → official НРД name (клинична пътека / амбулаторна / клинична
// процедура). Static reference (~40 KB) generated by
// scripts/nzok/write_procedure_names.ts; the activity feed carries only the code.
// Returns the flat { code: name } map (or null / empty until the pipeline populates
// it); the tiles resolve via resolveProcedureName in @/lib/nzokProcedures.
export const useNzokProcedureNames = () =>
  useQuery({
    queryKey: ["budget", "nzok", "procedure-names"] as const,
    queryFn: async () => {
      const f = await fetchJson<NzokProcedureNamesFile>(
        "/budget/nzok/procedures.json",
      );
      return f?.names ?? null;
    },
    staleTime: Infinity,
