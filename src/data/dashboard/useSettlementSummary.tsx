...
import { useSettlementVotes } from "../settlements/useSettlementVotes";
import { useSettlementStats } from "../settlements/useSettlementStats";
...
export const useSettlementSummary = (
  ...
  const { prevVotes } = useSettlementStats(ekatte);
  const { parties: partyInfos } = usePartyInfo();

  const data = useMemo<NationalSummary | undefined>(() => {
    if (!ekatte || !selected) return undefined;
    if (!settlement) return undefined;

    const protocol = settlement.results.protocol;
    const currentVotes = settlement.results.votes;
  ...
  }, [ekatte, selected, settlement, prevVotes, partyInfos, priorElections]);

  return {
    data,
    isLoading: !settlement || (!!ekatte && !partyInfos),
  };
};
