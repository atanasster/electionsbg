...
import { useSofiaStats } from "../country/useSofiaStats";
...
export const useSofiaSummary = (): {
  data?: NationalSummary;
  isLoading: boolean;
} => {
  const { selected, priorElections } = useElectionContext();
  const { votesSofia, votes } = useRegionVotes();
  const { prevVotes } = useSofiaStats();
  ...
  const data = useMemo<NationalSummary | undefined>(() => {
    ...
    if (!sofia) return undefined;

    const protocol = sofia.results.protocol;
    const currentVotes = sofia.results.votes;
  ...
  }, [selected, votes, votesSofia, prevVotes, partyInfos, priorElections]);

  return {
    data,
    isLoading: !votes || !partyInfos,
  };
};
