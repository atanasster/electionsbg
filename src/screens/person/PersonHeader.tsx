// The person-page header — identity + party + (for MPs) a compact one-line bio. Brings back
// the old candidate page's richness (party badge, parliament bio) at a fraction of its height.
...
export const PersonHeader: FC<{ p: PersonProfile; mpId: number | null }> = ({
  ...
  const { rows, dataCycles } = usePersonDataCycles(p.slug);
  ...
  return (
    <div className="flex items-start gap-4">
      <MpAvatar name={p.name} mpId={mpId} className="h-20 w-20 shrink-0" />
      <div className="min-w-0">
        ...
        {mpId != null && (
          <CandidateMpProvider
            value={{ id: mpId, name: p.name, entry: mpEntry ?? null }}
          >
            <MpProfileHeader name={p.name} compact />
          </CandidateMpProvider>
        )}
        {p.aliases.length > 0 && (
