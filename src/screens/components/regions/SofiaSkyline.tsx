// The "SOFIA" skyline silhouette used as a corner map shortcut.
//
// Presentational only — extracted from SofiaCity so both the parliamentary
// regions map (SofiaCity → /sofia) and the local-elections maps
// (LocalSofiaCityLink → /local/:cycle/SOF) can share the same artwork. The
// fill follows the leading party's colour; the caller supplies size + the
// link/tooltip wrapper.

import { FC } from "react";

export const SofiaSkyline: FC<{
  fillColor?: string;
  width: number;
  height: number;
  className?: string;
}> = ({ fillColor, width, height, className }) => (
  <svg
    ...
    height={`${height}px`}
  >
    <g className="stroke-black" strokeWidth={2} fill={fillColor || "lightgrey"}>
      <path
