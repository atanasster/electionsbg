...
>): Promise<PartyResultsRow[] | null> => {
  ...
  const response = await fetch(
    dataUrl(`/${queryKey[1]}/parties/by_municipality/${queryKey[2]}.json`),
  );
  const data = await response.json();
  return data;
};
export const PartyRecountByMunicipality: FC<{ party: PartyInfo }> = ({
  party,
}) => {
  const { t } = useTranslation();
  const { selected } = useElectionContext();
  const { data } = useQuery({
    queryKey: ["party_votes_by_municipality", selected, party?.number],
    queryFn,
  });
  return (
    <PartyRecountTable
      title={t("votes_recount_by_municipality")}
