// Top contracts preview for an awarder. Reads the pre-computed topContracts
// slice already embedded inside the awarder rollup — so the preview costs no
// extra fetch on top of the rollup the page already loads, instead of
// pulling the 4 MB awarder_contracts/<eik>.json just to render 10 rows.
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2 flex-wrap">
          ...
          {t("awarder_top_contracts") || "Top contracts"}
          <span className="text-xs text-muted-foreground font-normal ml-1">
            {t("awarder_top_contracts_subtitle") || "Largest by signed amount."}
          </span>
          <Link
            to={`/awarder/${eik}/contracts`}
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="text-left px-3 py-2">
                  {t("awarder_contract_contractor") || "Contractor"}
                </th>
