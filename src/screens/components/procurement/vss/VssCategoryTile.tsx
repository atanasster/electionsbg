// "Какво купува ВСС — по функция" — the ВСС's ЗОП spend folded into operationally
// meaningful buckets (courthouse construction, the e-justice IT backbone, energy,
// services & insurance, furnishings, other). Each row carries its € share,
// dominant supplier and single-bid share. Pure from VssCategoryAgg. Mirrors the
// НОИ/НЗОК tiles.
...
const CATEGORY_COLOR: Record<VssCategory, string> = {
  ...
  furnishing: "bg-teal-500",
  other: "bg-muted-foreground/50",
};

export const VssCategoryTile: FC<{
  categories: VssCategoryAgg[];
  totalEur: number;
}> = ({ categories, totalEur }) => {
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          {bg
            ? "Какво купува ВСС — по функция"
            : "What the ВСС buys — by function"}
        </CardTitle>
      </CardHeader>
