...
export const ProcurementFlowSankey: FC<{
  ...
  const typeLabel = (type: ProcurementFlowNodeType): string =>
    t(TYPE_I18N[type]) || type;
      ...
      >
          ...
          {layout.links.map((link, i) => {
            ...
            return (
              ...
              >
                ...
                <stop offset="100%" stopColor={TYPE_COLOR[tgt.type]} />
              </linearGradient>
            );
          })}
        </defs>
        <g>
          {layout.links.map((link, i) => {
            ...
            const tgt = link.target as SankeyNode<NodeDatum, LinkDatum>;
            const touchesFocus =
              focusId && (s.id === focusId || tgt.id === focusId);
            const opacity = focusId ? (touchesFocus ? 0.75 : 0.08) : 0.45;
            const tipContent = (
              <div className="flex flex-col gap-1.5 max-w-[260px]">
                <FlowEndpoint id={s.id} type={s.type} label={s.label} />
                <FlowEndpoint id={tgt.id} type={tgt.type} label={tgt.label} />
        ...
        <g>
          {layout.nodes.map((node) => {
            ...
            // with their neighbours. The hover tooltip on the rect still works,
...
// One labelled row in the hover tooltip. MP endpoints get the shared avatar
// (photo + party-coloured ring); awarders/contractors get a plain colour chip.
const FlowEndpoint: FC<{
  id: string;
