...
export const RegionalConvergenceTile: FC<{ oblasts: RegionalOblastAgg[] }> = ({
  ...
  const bg = lang === "bg";
  // Shared tooltip (never a hand-rolled one, never the native <title>): it clamps to
  // the viewport and handles the flicker loop. {tooltip} is rendered OUTSIDE the Card
  // because it positions with page coordinates + position:absolute.
  ...
  return (
    <>
      ...
      {/* OUTSIDE the Card: the shared tooltip positions with page
          coordinates + position:absolute (see @/ux/useTooltip). */}
      {tooltip}
    </>
  );
};
