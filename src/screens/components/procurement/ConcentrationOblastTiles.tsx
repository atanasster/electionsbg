...
// that oblast (NUTS3). Clicking a tile opens the full explorer filtered to that
// oblast. A separate "national / unresolved" tile covers buyers with no single
// seat (central ministries/agencies), so the map never silently drops them.
...
export const ConcentrationOblastTiles: FC<{
  byOblast: Array<{ nuts: string; count: number }>;
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-7 gap-1.5">
        {byOblast.map((o) => (
          <Tile
            key={o.nuts}
            label={nuts3Name(o.nuts, lang)}
            count={o.count}
            tier={tierOf(o.count, max)}
            to={`/procurement/concentration?oblast=${o.nuts}`}
          />
        ))}
        {nationalCount > 0 && (
          <Tile
