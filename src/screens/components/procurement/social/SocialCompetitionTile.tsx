// "Къде се къса конкуренцията" — per-unit single-bid share across the social
// group's ЗОП spend: bar length = € contracted, colour green→amber→red by the share
// of contracts let with a single bidder. Pure from the SocialUnitAgg rollup.
// Mirrors MvrCompetitionTile.
//
// FRAMING: single-bid is a RISK SIGNAL, not a verdict — some procurement is
// legitimately sole-source. Labelled as "worth a look".
...
import type { SocialUnitAgg } from "@/data/procurement/useSocial";
...
export const SocialCompetitionTile: FC<{ units: SocialUnitAgg[] }> = ({
  units,
}) => {
    ...
    <Card id="competition">
      ...
      <CardContent className="p-3 md:p-4 space-y-2">
        ...
        <p className="pt-1 text-[11px] text-muted-foreground/80">
          {bg
            ? `Дял на договорите с една оферта, за структури с поне ${MIN_BID_KNOWN} договора с известен брой оферти. Дебелината показва договорената стойност; цветът — конкуренцията (зелено под 35%, червено над 60%). Сигнал за риск, не присъда.`
            : `Single-bidder share, for units with at least ${MIN_BID_KNOWN} bid-known contracts. Bar length is contracted value; colour is competition (green below 35%, red above 60%). A risk signal, not a verdict.`}
        </p>
      </CardContent>
    </Card>
  );
