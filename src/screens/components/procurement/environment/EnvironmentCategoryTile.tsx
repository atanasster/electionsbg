// „Какво купува МОСВ — по функция" — the group's ЗОП spend folded into environmental
...
export const EnvironmentCategoryTile: FC<{
  categories: EnvCategoryAgg[];
  totalEur: number;
}> = ({ categories, totalEur }) => {
  ...
  const categoryHref = (id: EnvCategory): string | null => {
    ...
    <Card id="function">
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2">
          ...
          {bg
            ? "Какво купува МОСВ — по функция"
            : "What МОСВ buys — by function"}
        </CardTitle>
      </CardHeader>
      <CardContent className="p-3 md:p-4 space-y-2.5">
        ...
        <p className="pt-1 text-[11px] text-muted-foreground/80">
          {bg
            ? "Категориите групират CPV-разделите на договорите в екологични функции (класификация, не официална таксономия). Делът с една оферта се показва за категории с поне 3 договора с известен брой оферти."
            : "Categories group the contracts' CPV divisions into environmental functions (a classification, not an official taxonomy). Single-bid share is shown for categories with at least 3 bid-known contracts."}
        </p>
      </CardContent>
    </Card>
  );
