// /procurement/concentration — the full single-supplier-concentration list
// behind the top-20 excerpt on /procurement/flags. Every flagged buyer→supplier
// pair (≥30% of the buyer's lifetime spend on one supplier, buyer ≥ €100k),
// searchable, sortable, filterable by the buyer's oblast (?oblast=BG4xx, or
// =national for central/unresolved buyers — the link the flags tile-map opens),
// with CSV export. A public-record fact per row, not an accusation.
...
export const ProcurementConcentrationScreen: FC = () => {
  ...
  return (
    <div>
      <Title
        description={
          t("concentration_desc") ||
          "Every buyer that put 30%+ of its lifetime spending on a single supplier (buyer total ≥ €100k)."
        }
      >
      ...
      {isLoading || !data ? (
        ...
        <>
          {/* Controls */}
          <div className="mb-3 mt-4 flex flex-wrap items-center gap-2 text-sm">
            <input
              type="search"
              value={query}
