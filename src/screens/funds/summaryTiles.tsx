// Shared tiles for the ИСУН drill-down pages — /funds/programme/{code} and
// /funds/procedure/{code}. The two levels render the same five blocks over the
// same shard shape, so they share the components rather than keeping two copies
// that would drift apart on the next status or palette change.

import { FC } from "react";
import { Link } from "react-router-dom";
import { useTranslation } from "react-i18next";
import { Activity, Coins, MapPin, Users } from "lucide-react";
...
// The tiles serve two grains — programme and procedure — whose shards carry
// identical row shapes. Aliased off the programme file so the props do not read
// as programme-only at the procedure call sites.
type StatusRows = FundsProjectsProgramSummaryFile["statusBreakdown"];
type BeneficiaryRows = FundsProjectsProgramSummaryFile["topBeneficiaries"];
type ContractRows = FundsProjectsProgramSummaryFile["topContracts"];
type MuniRows = FundsProjectsProgramSummaryFile["topMunis"];
...
export const HeaderKpis: FC<{ rollup: FundsProjectsRollup }> = ({ rollup }) => {
  const { t } = useTranslation();
  const rate =
...
export const TopBeneficiaries: FC<{
      ...
      <CardContent className="p-3 md:p-4">
        <ul className="divide-y divide-border/60">
          {rows.map((m, i) => {
            ...
            // detail pages mirror the choropleth's Sofia handling.
