// What this tile must never draw without, and the two states its copy has to tell apart.
...
describe("PresidentialNeighborhoodsTile", () => {
  ...
  it("links every district to the report that named it", () => {
    ...
    // as its own.
    mount(neighborhoods());
    const link = screen.getByText(bgCorpus.presidential_hoods_source);
    expect(link.getAttribute("href")).toBe("https://www.segabg.com/hot/x");
    expect(link.getAttribute("rel")).toContain("noopener");
  });

  it("puts each ticket's share beside its NATIONAL share", () => {
    // A share inside eight districts means nothing without what the country did.
    // ⚠ THE FORMATTERS, NOT LITERALS. The Bulgarian corpus drops a trailing decimal zero, so
  ...
  it("says WHY the invalid rate is missing rather than printing a dash", () => {
    ...
    mount(
      neighborhoods({
        totals: rates({ invalidPct: null, paperBallots: 228 }),
      }),
    );
  ...
  it("names its basis as the DOMESTIC sections, not the country at large", () => {
    ...
    // same page — 40,3% against this 37,2% on 2021 round 1. Two irreconcilable „national
    // turnout" figures on one page is a credibility problem even when the qualified one is the
  ...
  it("announces the tickets it does not show", () => {
    ...
    const one = neighborhoods().tickets[0];
    mount(
      neighborhoods({
        tickets: Array.from({ length: 8 }, (_, i) => ({
          ...one,
          number: i + 1,
          president: `Кандидат ${i + 1}`,
        })),
      }),
    );
    expect(
      screen.getByText(
        bgCorpus.presidential_hoods_more_tickets.replace("{{count}}", "2"),
      ),
    ).toBeTruthy();
