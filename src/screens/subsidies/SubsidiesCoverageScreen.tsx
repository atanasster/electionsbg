...
export const SubsidiesCoverageScreen: FC = () => {
  ...
  const { data } = useAgriOverview("all");
    ...
    <>
      ...
      <section aria-label={title} className="my-4">
        ...
        <p className="mt-4 text-xs text-muted-foreground">
          ...
          {bg
            ? `${(data?.years ?? []).length} покрити финансови години, ${data ? formatEur(data.headline.totalEur, L) : "—"} общо`
            : `${(data?.years ?? []).length} financial years covered, ${data ? formatEur(data.headline.totalEur, L) : "—"} in total`}{" "}
          ·{" "}
          <a
