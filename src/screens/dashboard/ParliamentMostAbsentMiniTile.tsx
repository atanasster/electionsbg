...
export const ParliamentMostAbsentMiniTile: FC = () => {
  const { t, i18n } = useTranslation();
  const { mostAbsent, isLoading } = useAttendanceRanking(PREVIEW, PREVIEW);
  ...
  if ((isLoading || mpsLoading) && mostAbsent.length === 0) {
    return (
      <Card aria-hidden>
        <CardContent>
  ...
  if (mostAbsent.length === 0) return null;
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2 flex-wrap">
          ...
          {t("hub_most_absent_title") || "Most absent MPs"}
          <Link
      ...
      <CardContent>
        <div className="text-xs text-muted-foreground mb-3">
          {t("hub_most_absent_lede") ||
            "MPs with the highest share of roll-call items where they were recorded as absent."}
        </div>
        <ul className="divide-y">
          {mostAbsent.map((e) => {
            ...
            const absentFrac = 1 - e.presentPct;
            return (
              <li key={e.mpId}>
                <Link
