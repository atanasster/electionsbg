...
export const TopMunicipalitiesTile: FC<Props> = ({ parties, regionCode }) => {
  ...
  const rows = useMemo(() => {
    ...
    const enriched = data
      .map((m) => {
        ...
        const machineVotes = m.results.votes.reduce(
        ...
        const info = findMunicipality(m.obshtina);
        const name =
          (i18n.language === "bg"
  ...
  }, [data, findMunicipality, i18n.language, partyColorMap]);
    ...
    <StatCard
      label={
        <div className="flex items-center justify-between w-full">
          ...
          {totalCount > TOP_N ? (
            <Link
              to={`/municipality/${regionCode}/municipalities`}
