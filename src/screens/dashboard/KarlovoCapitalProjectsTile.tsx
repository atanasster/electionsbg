// Per-settlement breakdown of Карлово's annual капиталова програма.
//
// Karlovo (PDV13) is a Plovdiv-oblast município with 27 settlements
// (4 towns: Карлово, Калофер, Клисура, Баня + 23 villages). Source is
// a clean XLSX served by karlovo.bg via the service-download-file.php
// endpoint, discovered via the council's current-news page. No OCR.
...
import { useKarlovoCapitalProgram } from "@/data/budget/useBudget";

const KRL_CAPITAL_YEARS = [2025] as const;
const KRL_CAPITAL_LATEST_YEAR = KRL_CAPITAL_YEARS[0];
const KRL_OBSHTINA = "PDV13";
...
export const KarlovoCapitalProjectsTile: FC<{ obshtinaCode: string }> = ({
  obshtinaCode,
}) => {
  const { t, i18n } = useTranslation();
  const lang = i18n.language.startsWith("bg") ? "bg" : "en";
  const enabled = obshtinaCode === KRL_OBSHTINA;
  const [year, setYear] = useState<number>(KRL_CAPITAL_LATEST_YEAR);
  const { data, isLoading } = useKarlovoCapitalProgram(
    ...
    <Card>
      ...
      <CardContent className="p-3 md:p-4 space-y-3">
        ...
        {topSettlements.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("karlovo_capital_by_settlement")}
            </div>
                  ...
                  >
                    <div className="grid grid-cols-[1fr_auto_auto] items-baseline gap-3">
                      ...
                      <span className="tabular-nums text-muted-foreground w-16 text-right shrink-0">
                        {t("karlovo_capital_project_count_compact", {
        ...
        {topProjects.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("karlovo_capital_top_projects")}
            </div>
            <div className="space-y-1">
              {topProjects.map((p) => (
                <div
