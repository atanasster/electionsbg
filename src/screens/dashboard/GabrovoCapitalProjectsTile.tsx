// Per-settlement breakdown of Габрово's annual investment programme.
//
// Gabrovo (GAB05) is an oblast capital with 134 settlements (the city
// + 133 villages — the largest village count in the fleet). Source is
// a 9-page born-digital landscape PDF on gabrovo.bg (Приложение №5 to
// the budget). OCR via Gemini Vision.

import { FC, useState } from "react";
import { useTranslation } from "react-i18next";
import { HardHat } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/ux/Card";
import { useGabrovoCapitalProgram } from "@/data/budget/useBudget";

const GAB_CAPITAL_YEARS = [2025] as const;
const GAB_CAPITAL_LATEST_YEAR = GAB_CAPITAL_YEARS[0];
const GAB_OBSHTINA = "GAB05";
...
export const GabrovoCapitalProjectsTile: FC<{ obshtinaCode: string }> = ({
  obshtinaCode,
}) => {
  const { t, i18n } = useTranslation();
  const lang = i18n.language.startsWith("bg") ? "bg" : "en";
  const enabled = obshtinaCode === GAB_OBSHTINA;
  const [year, setYear] = useState<number>(GAB_CAPITAL_LATEST_YEAR);
  const { data, isLoading } = useGabrovoCapitalProgram(
      ...
      <CardContent className="p-3 md:p-4 space-y-3">
        <div className="flex items-baseline gap-3 flex-wrap">
          ...
          <span className="text-xs text-muted-foreground">
            {t("gabrovo_capital_project_count", {
        ...
        {topSettlements.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("gabrovo_capital_by_settlement")}
            </div>
                  ...
                  >
                    <div className="grid grid-cols-[1fr_auto_auto] items-baseline gap-3">
                      ...
                      <span className="tabular-nums text-muted-foreground w-16 text-right shrink-0">
                        {t("gabrovo_capital_project_count_compact", {
        ...
        {topProjects.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("gabrovo_capital_top_projects")}
            </div>
            <div className="space-y-1">
              {topProjects.map((p) => (
                <div
