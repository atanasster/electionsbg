...
const mpLinkedCount: { current: number } = { current: 0 };
vi.mock("@/data/parliament/usePlaceCompanies", () => ({
  usePlaceCompanies: () => ({ data: placeData.current, isLoading: false }),
}));
vi.mock("@/data/parliament/useCompaniesAtSettlement", () => ({
  useCompaniesHqSummary: () => ({
    data: mpLinkedCount.current ? { count: mpLinkedCount.current } : null,
  }),
}));
...
const renderTile = (data: PlaceCompanies | null, mpLinked = 0) => {
  placeData.current = data;
  mpLinkedCount.current = mpLinked;
  return render(
    <MemoryRouter>
...
describe("PlaceCompaniesTile", () => {
  ...
  it("offers the MP-linked page only when that shard has rows", () => {
    renderTile(village(), 0);
    expect(
      screen.queryByText(/place_companies_see_mp_linked/),
    ).not.toBeInTheDocument();
    renderTile(village(), 3);
    expect(
