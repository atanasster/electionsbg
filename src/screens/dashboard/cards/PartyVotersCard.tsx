...
export const PartyVotersCard: FC<Props> = ({ data }) => {
  const { t } = useTranslation();
  const delta = data.deltaPctNational;
  ...
  return (
    <StatCard label={t("voters")} hint={t("dashboard_party_voters_hint")}>
      ...
      {delta !== undefined && (
        <div className={`text-sm font-medium tabular-nums ${accent}`}>
          ...
          {sign}
          {formatPct(delta, 2)} {t("dashboard_pct_points")}
          {data.priorElection && (
