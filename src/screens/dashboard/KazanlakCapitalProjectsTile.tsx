// Per-settlement breakdown of Казанлък's annual капиталова програма.
//
// Kazanlak (SZR12) is a Stara Zagora-oblast município with 20 settlements
// (3 towns: Казанлък, Крън, Шипка + 17 villages). Source is the council's
// "Приложения" PDF (Приложение №4 — Проект на инвестиционна програма и
// текущи ремонти за 2025 г.), discovered via the kazanlak.bg Nuxt
// _payload.json since the site is JS-rendered. OCR via Gemini Vision.
...
import { useKazanlakCapitalProgram } from "@/data/budget/useBudget";

const KZN_CAPITAL_YEARS = [2025] as const;
const KZN_CAPITAL_LATEST_YEAR = KZN_CAPITAL_YEARS[0];
const KZN_OBSHTINA = "SZR12";
...
export const KazanlakCapitalProjectsTile: FC<{ obshtinaCode: string }> = ({
  obshtinaCode,
}) => {
  const { t, i18n } = useTranslation();
  const lang = i18n.language.startsWith("bg") ? "bg" : "en";
  const enabled = obshtinaCode === KZN_OBSHTINA;
  const [year, setYear] = useState<number>(KZN_CAPITAL_LATEST_YEAR);
  const { data, isLoading } = useKazanlakCapitalProgram(
      ...
      <CardContent className="p-3 md:p-4 space-y-3">
        <div className="flex items-baseline gap-3 flex-wrap">
          ...
          <span className="text-xs text-muted-foreground">
            {t("kazanlak_capital_project_count", {
        ...
        {topSettlements.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("kazanlak_capital_by_settlement")}
            </div>
                  ...
                  >
                    <div className="grid grid-cols-[1fr_auto_auto] items-baseline gap-3">
                      ...
                      <span className="tabular-nums text-muted-foreground w-16 text-right shrink-0">
                        {t("kazanlak_capital_project_count_compact", {
        ...
        {topProjects.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("kazanlak_capital_top_projects")}
            </div>
            <div className="space-y-1">
              {topProjects.map((p) => (
                <div
        ...
        <p className="text-[11px] text-muted-foreground">
          {t("kazanlak_capital_tile_caveat")}
        </p>
      </CardContent>
    </Card>
  );
