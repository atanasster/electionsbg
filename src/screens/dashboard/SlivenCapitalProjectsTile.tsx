// Per-settlement breakdown of Сливен's annual капиталова програма.
//
// Sliven isn't районирана; it's a Tier-2 oblast capital (SLV20) with 45
// settlements (the city + the town Кермен + 43 villages). UX mirrors
// Stara Zagora: total + per-settlement strip + top 5 city projects.
//
// Mounted on the Sliven settlement page (EKATTE 67338) and the
...
import { useSlivenCapitalProgram } from "@/data/budget/useBudget";

const SLIVEN_CAPITAL_LATEST_YEAR = 2025;
const SLIVEN_OBSHTINA = "SLV20";
...
export const SlivenCapitalProjectsTile: FC<{ obshtinaCode: string }> = ({
  obshtinaCode,
}) => {
  const { t, i18n } = useTranslation();
  const lang = i18n.language.startsWith("bg") ? "bg" : "en";
  const enabled = obshtinaCode === SLIVEN_OBSHTINA;
  const { data, isLoading } = useSlivenCapitalProgram(
    enabled ? SLIVEN_CAPITAL_LATEST_YEAR : undefined,
  );
      ...
      <CardContent className="p-3 md:p-4 space-y-3">
        <div className="flex items-baseline gap-3 flex-wrap">
          ...
          <span className="text-xs text-muted-foreground">
            {t("sliven_capital_project_count", { count: data.projects.length })}
          </span>
        </div>
        ...
        {topSettlements.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("sliven_capital_by_settlement")}
            </div>
                  ...
                  >
                    <div className="grid grid-cols-[1fr_auto_auto] items-baseline gap-3">
                      ...
                      <span className="tabular-nums text-muted-foreground w-16 text-right shrink-0">
                        {t("sliven_capital_project_count_compact", {
