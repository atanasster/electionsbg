// Per-settlement breakdown of Самоков's annual капиталова програма.
//
// Samokov (SFO39) is a Sofia-oblast município with 28 settlements
// (city + 27 villages, including resort к.к. Боровец). Source is a
// 10-page born-digital PDF on samokov.bg discovered via Google. OCR
// via Gemini Vision.
...
const SAM_CAPITAL_LATEST_YEAR = SAM_CAPITAL_YEARS[0];
const SAM_OBSHTINA = "SFO39";
...
export const SamokovCapitalProjectsTile: FC<{ obshtinaCode: string }> = ({
  obshtinaCode,
}) => {
  const { t, i18n } = useTranslation();
  const lang = i18n.language.startsWith("bg") ? "bg" : "en";
  const enabled = obshtinaCode === SAM_OBSHTINA;
  const [year, setYear] = useState<number>(SAM_CAPITAL_LATEST_YEAR);
  const { data, isLoading } = useSamokovCapitalProgram(
      ...
      <CardContent className="p-3 md:p-4 space-y-3">
        <div className="flex items-baseline gap-3 flex-wrap">
          ...
          <span className="text-xs text-muted-foreground">
            {t("samokov_capital_project_count", {
        ...
        {topSettlements.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("samokov_capital_by_settlement")}
            </div>
                  ...
                  >
                    <div className="grid grid-cols-[1fr_auto_auto] items-baseline gap-3">
                      ...
                      <span className="tabular-nums text-muted-foreground w-16 text-right shrink-0">
                        {t("samokov_capital_project_count_compact", {
        ...
        {topProjects.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("samokov_capital_top_projects")}
            </div>
            <div className="space-y-1">
              {topProjects.map((p) => (
                <div
