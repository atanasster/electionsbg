// Per-settlement breakdown of Кърджали's annual капиталова програма.
//
// Kardzhali (KRZ16) is an oblast capital with 118 settlements (city +
// 117 villages). Source PDF discovered via Google site:kardjali.bg;
// born-digital Excel-rendered PDF; OCR via Gemini Vision.

import { FC, useState } from "react";
import { useTranslation } from "react-i18next";
import { HardHat } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/ux/Card";
import { useKardzhaliCapitalProgram } from "@/data/budget/useBudget";

const KRJ_CAPITAL_YEARS = [2025, 2024] as const;
const KRJ_CAPITAL_LATEST_YEAR = KRJ_CAPITAL_YEARS[0];
const KRJ_OBSHTINA = "KRZ16";
...
export const KardzhaliCapitalProjectsTile: FC<{ obshtinaCode: string }> = ({
  obshtinaCode,
}) => {
  const { t, i18n } = useTranslation();
  const lang = i18n.language.startsWith("bg") ? "bg" : "en";
  const enabled = obshtinaCode === KRJ_OBSHTINA;
  const [year, setYear] = useState<number>(KRJ_CAPITAL_LATEST_YEAR);
  const { data, isLoading } = useKardzhaliCapitalProgram(
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2 flex-wrap">
          ...
          {t("kardzhali_capital_tile_title")}
          <select
            value={year}
        ...
        <p className="text-xs text-muted-foreground">
          {t("kardzhali_capital_tile_intro")}
        </p>
      </CardHeader>
      <CardContent className="p-3 md:p-4 space-y-3">
        <div className="flex items-baseline gap-3 flex-wrap">
          ...
          <span className="text-xs text-muted-foreground">
            {t("kardzhali_capital_project_count", {
        ...
        {topSettlements.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("kardzhali_capital_by_settlement")}
            </div>
                  ...
                  >
                    <div className="grid grid-cols-[1fr_auto_auto] items-baseline gap-3">
                      ...
                      <span className="tabular-nums text-muted-foreground w-16 text-right shrink-0">
                        {t("kardzhali_capital_project_count_compact", {
        ...
        {topProjects.length > 0 && (
          <div>
            <div className="text-xs font-medium mb-1">
              {t("kardzhali_capital_top_projects")}
            </div>
            <div className="space-y-1">
              {topProjects.map((p) => (
                <div
        ...
        <p className="text-[11px] text-muted-foreground">
          {t("kardzhali_capital_tile_caveat")}
        </p>
      </CardContent>
    </Card>
  );
