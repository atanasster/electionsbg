// /culture/subsidies — the НФЦ film-subsidy dashboard. This body used to BE
// /culture; it moved here when /culture became the sector hub, unchanged, because
// film subsidy is 13% of the money on this sector's subject and was 100% of its
// content. The hub fronts it with a tile.
//
// ⚠️ THE SEO OF THIS MOVE IS NOT AUTOMATIC. /culture carried the film copy and
// whatever it ranked for; `scripts/prerender/routes.ts` reads
// data/culture/overview.json at BUILD time and interpolates the totals into that
// body. The prerender entry moved with this file — if you are here because a
// figure in the indexed HTML looks wrong, that is where it comes from, not here.
//
// It owns the per-recipient subsidy story (Phase 1: НФЦ film money — who gets it,
// what discipline, how it concentrates, how it moved over time); the МК awarder
// pack (/awarder/000695160) owns the money-as-buyer sliver, and the МК ministry
// page owns the budget.
//
...
export const CultureSubsidiesScreen = () => {
  ...
  return (
    <>
      ...
      <SectorBreadcrumb
        ...
        current={bg ? "Филмови субсидии" : "Film subsidies"}
      />
      ...
      {data && scoped && (
        <div className="space-y-4">
          ...
          {/* The /culture hub's „Кой решава" tile links to #commissions. An
              anchor with no element scrolls nowhere and reads as a broken tile —
              `cultureRegistry.test.ts` asserts every fragment a tile emits exists
              in the destination's source. */}
          <div id="commissions" className="scroll-mt-20">
