...
// The oblast → region-page rule (and its "null rather than a dead link" contract) lives in
// placeViews.ts with the other place-URL builders, and is tested there: a /person office
// row's МИР badge needs the same fold, and two copies of "which oblast codes have a page"
// would drift.
import { oblastGovernanceUrl } from "@/data/local/placeViews";
...
export const buildOblastRows = (
  ...
  return byOblast.map((o) => {
    ...
    return {
      oblast: o.oblast,
      name: name(o.oblast),
      href: oblastGovernanceUrl(o.oblast),
