...
//
// NO EMPTY GUARD, deliberately: four of the five pickers are unconditional and the three
// toggles always render, so `selects.length === 0 && toggles.length === 0` is unreachable — a
// branch that cannot be entered is a branch nobody maintains and no test can cover.
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-end gap-x-3 gap-y-3">
        {selects.map((s) => {
          const labelId = `persons-filter-${s.key}-${uid}`;
          return (
