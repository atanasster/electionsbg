...
export const e2: ExplainerSpec<RiskCanvasState> = {
  ...
  scenes: [
    ...
    },
    {
      ...
      body: "Тези гласове не са сгрешени.\nПросто не могат да се проверят.",
      voiceOver:
        "Скалата тук свършва на един процент, така че оценката е петдесет и девет. Тези гласове не са сгрешени — просто не могат да се проверят.",
    ...
    // ── ЧАСТ 10 · Контекстните пет ───────────────────────────────────────────
    {
      ...
      voiceOver:
        "Остават петте показателя, които стоят до числото, но не влизат в него. Средната им стойност е четирийсет. За всеки от тях ще кажем и къде стои спрямо предишните избори.",
    ...
    },
    {
      ...
      body: "Тест за разпределението на цифрите.\nВ 11 от 13 измерени избора тук няма нито една партия.",
      voiceOver:
        "Първият е тестът на Бенфорд, който гледа разпределението на цифрите в резултатите. Само една партия от дванайсет се отклонява силно. За сравнение — в единайсет от тринайсетте измерени избора няма нито една такава партия.",
      onScreen: "1 / 12",
      grounding: {
        file: "video/src/generated/risk.json",
    ...
    },
    {
      ...
      body: "Осем следени квартала, сравнени със страната.\nВтората най-голяма разлика, която сме мерили.",
      voiceOver:
        "Вторият следи осем ромски квартала. Първата партия там се е повишила с пет цяло и девет пункта повече, отколкото се е повишила в цялата страна. Второто най-голямо разминаване от единайсет измервания — почти колкото през юни две хиляди двайсет и четвърта.",
    ...
    },
    {
      ...
      body: "Шест избора са на същия таван от 100.\nЧислото под оценката ги различава.",
      voiceOver:
        "Тук оценката не помага — шест избора опират в същия таван от сто. Затова гледаме числото под нея: четирийсет и девет цяло и седем, второто най-високо от дванайсет измервания. По-високо е било само през две хиляди и девета.",
      onScreen: "49,7 · 2-ро",
      grounding: {
        file: "video/src/generated/risk.json",
        path: "$.facts.context.voteSwitching",
    ...
    },
    {
      id: 53,
      kicker: "Но",
      headline: "Мери разместване,\nне преброяване",
      body: "Затова стои настрани от оценката.",
      voiceOver:
        "Той обаче описва колко се е разместил вотът между партиите, а не как е бил преброен. Затова стои настрани от оценката.",
    ...
    },
    {
      ...
      body: "Второто най-голямо от 9 измервания.\nВ пет от тях агенциите остават под прага.",
      voiceOver:
        "Четвъртият е средната грешка на социологическите агенции — две цяло петдесет и един пункта. Това е второто най-голямо отклонение от девет измервания; в пет от тях агенциите остават под прага и показателят е нула.",
    ...
    },
    {
      ...
      body: "Маркирани секции, съседни една на друга.\nЕдинайсето място от 13 — почти най-ниското.",
      voiceOver:
        "Петият гледа дали маркираните секции са съседни. Сто и осем от хиляда двеста шейсет и три попадат в такива струпвания, или осем цяло и шест процента. Тук поредицата е обратна — това е единайсето място от тринайсет, тоест почти най-ниското, което сме мерили.",
    ...
    // ── ЧАСТ 11 · Какво не казва ─────────────────────────────────────────────
    {
      ...
      voiceOver:
        "Ако трябва да се обобщи контекстът — четири от петте стоят високо в собствената си поредица, а клъстерите са близо до дъното. Три от високите обаче описват едно и също: партия, която се появява отникъде и разбърква вота.",
      onScreen: "4 високи · 1 нисък",
      grounding: {
        file: "video/src/generated/risk.json",
        path: "$.facts.contextStanding",
      },
    },
    {
      id: 58,
      kicker: "И най-важното",
      headline: "Гледайте поредицата,\nне едно число",
      body: "Индексът се смята за всички избори.\nЕдна стойност сама по себе си не значи нищо.",
      voiceOver:
        "И най-важното — индексът се смята за всички избори назад. Едно число само по себе си не значи нищо; значение има къде стои спрямо останалите.",
      onScreen: "поредица, не число",
    },
    {
      id: 59,
      canvas: { dim: 1 },
