...
test("story cards preserve responsive, focus and motion contracts", async ({
  ...
  for (const width of VIEWPORTS) {
    ...
    await expect(page.locator(".news-story-card")).toHaveCount(4);
    expect(
      await page.evaluate(() => ({
  ...
  const imageCardHeight = await page
    .getByTestId("image-led-single-source")
    .locator(".news-story-card")
    .evaluate((card) => card.getBoundingClientRect().height);
  const textCardHeight = await page
    .getByTestId("text-first-multi-source")
    .locator(".news-story-card")
    .evaluate((card) => card.getBoundingClientRect().height);
  expect(textCardHeight).toBeLessThan(imageCardHeight);
