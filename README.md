...
| `funds/` | EU-funds beneficiary register from ИСУН 2020 (`2020.eufunds.bg`): `index.json` (corpus totals, by-organisation-type and by-public/private-law breakdowns, top beneficiaries) plus `beneficiaries/<0-9>.json` + `_x.json` shards keyed on EIK last digit — one row per organisation with contracts signed, funds contracted and funds actually paid, all EUR |
...
- `funds/` — ИСУН EU-funds beneficiary ingest. `ingest.ts` downloads the public "Бенефициенти" XLSX export from `2020.eufunds.bg` (cached under `data/_cache/funds/`), `parse.ts` locates the table by header match and flattens the ~53k organisation rows into `FundsBeneficiary` records, then the ingest shards them by EIK last digit and rebuilds `data/funds/index.json`. Full rebuild every run — the export is an all-time per-organisation rollup, not an event feed — with a header-schema guard + row-count floor in lieu of a canary.
...
- `watch/` — Tier-1 daily watcher (25 upstream sources fingerprint-diffed → daily markdown report under `data-reports/` + per-source state under `state/watch/`, see "Continuous data refresh" below)
...
| `update-funds` | Ingest the ИСУН 2020 EU-funds beneficiary register from `2020.eufunds.bg` into `data/funds/`. Full re-export each run (all-time per-organisation rollups: contracts signed, funds contracted, funds paid); header-schema guard + 40k row-count floor in lieu of a canary; the beneficiary EIK is captured (~87% coverage) as the join surface for a future MP cross-reference. |
