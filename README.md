...
- **Per-município capital programmes** (Sofia, Plovdiv, Burgas, Stara Zagora, Ruse, Varna) — each община publishes its own annual капиталова програма (поименен списък на обектите за капиталови разходи) as a board-decision appendix, but the format varies per city: [Sofia](https://www.sofia.bg/) ships Приложение №3 as a clean XLSX with paragraph / function / activity hierarchy and 24 районi tagged in free text; [Plovdiv](https://www.plovdiv.bg/proekt-budget-2025/) ships a borderless PDF with vertical-text "Разпоредител с бюджет" column and 6 районi explicitly tagged; [Burgas](https://burgas.bg/bg/2025/proektobyudzhet-2025-g/) ships the капиталова програма as one sheet inside the city draft-budget XLSX workbook with 7 funding-source columns (state subsidy / own / debt / EU / other / 2× carry-overs) and 11 villages + named city quarters from [Wikipedia категория Квартали_на_Бургас](https://bg.wikipedia.org/wiki/Категория:Квартали_на_Бургас); [Stara Zagora](https://www.starazagora.bg/bg/obshtinski-byudzhet/byudzhet-za-2025-godina/) ships Приложение №4 as a born-digital PDF inside a council-decision ZIP, tagging 51 villages of obshtina СЗР31 via the "с." prefix; [Ruse](https://obshtinaruse.bg/razchet-za-kapitalovi-razhodi) ships a multi-sheet XLSX with the **highest data quality** of the bunch — one sheet per second-level spending unit + one dedicated sheet per kmetstvo for the 12 villages, so per-settlement attribution is via workbook structure rather than free-text regex; [Varna](https://varnacouncil.bg/) is the hardest — only 71-page rasterized scans, so the parser pipeline has an OCR pre-step via Gemini Vision (`scripts/budget/capital_programs/varna_ocr.ts`) that runs once per fiscal year and caches the extracted JSON under `raw_data/`, then the rollup parser is deterministic. Per-município parsers under `scripts/budget/capital_programs/{muni}.ts` write a common-shaped `data/budget/capital_programs/{year}/{muni}.json` (recap total + per-project list + per-район or per-village rollup), and five matching frontend tiles surface the data inside the "финанси на общината" section on each city's settlement and município page — each tile silent-no-ops for non-matching obshtina codes. The `capital_programs` watcher tracks all five URLs under one fingerprint; new years require adding the URL to both `CAPITAL_PROGRAM_URLS` in the watcher AND `SOURCE_URLS` in each parser. Note: Varna is the 5th районирана община but currently blocked — varnacouncil.bg publishes the capital programme only as 200dpi rasterized scans, requiring OCR (Gemini Vision or similar) to extract.
